   ...
   const SwitchVlanEntry *entry)
{
   ...
   uint_t i;
   uint8_t *p;
   Ksz8775VlanEntrySet vlanSet;

   //The VLAN table supports up to 4096 VLAN entries
   if(entry->vlanId < KSZ8775_VLAN_TABLE_SIZE)
   {
      //Select the VLAN table for reading
      ksz8775WriteSwitchReg(interface, KSZ8775_INDIRECT_CTRL0,
         KSZ8775_INDIRECT_CTRL0_READ |
         KSZ8775_INDIRECT_CTRL0_TABLE_SEL_VLAN);

      //Trigger the read operation
      ksz8775WriteSwitchReg(interface, KSZ8775_INDIRECT_CTRL1,
         entry->vlanId / 4);
      ...
      //Read indirect data registers
      for(i = 0; i < sizeof(Ksz8775VlanEntrySet); i++)
      {
         p[i] = ksz8775ReadSwitchReg(interface, KSZ8775_INDIRECT_DATA7 + i);
      }
