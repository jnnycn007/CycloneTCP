   ...
   const SwitchVlanEntry *entry)
{
   ...
   uint_t i;
   uint8_t *p;
   Ksz8794VlanEntrySet vlanSet;

   //The VLAN table supports up to 4096 VLAN entries
   if(entry->vlanId < KSZ8794_VLAN_TABLE_SIZE)
   {
      //Select the VLAN table for reading
      ksz8794WriteSwitchReg(interface, KSZ8794_INDIRECT_CTRL0,
         KSZ8794_INDIRECT_CTRL0_READ |
         KSZ8794_INDIRECT_CTRL0_TABLE_SEL_VLAN);

      //Trigger the read operation
      ksz8794WriteSwitchReg(interface, KSZ8794_INDIRECT_CTRL1,
         entry->vlanId / 4);
      ...
      //Read indirect data registers
      for(i = 0; i < sizeof(Ksz8794VlanEntrySet); i++)
      {
         p[i] = ksz8794ReadSwitchReg(interface, KSZ8794_INDIRECT_DATA7 + i);
      }
