/**
 * @file sc589_eth2_driver.c
 * @brief ADSP-SC589 Ethernet MAC driver (EMAC1 instance)
 *
 * @section License
 *
...
#include "core/net.h"
#include "drivers/mac/sc589_eth2_driver.h"
#include "debug.h"
...
#pragma location = SC589_ETH2_RAM_SECTION
static uint8_t txBuffer[SC589_ETH2_TX_BUFFER_COUNT][SC589_ETH2_TX_BUFFER_SIZE];
//Receive buffer
#pragma data_alignment = 4
#pragma location = SC589_ETH2_RAM_SECTION
static uint8_t rxBuffer[SC589_ETH2_RX_BUFFER_COUNT][SC589_ETH2_RX_BUFFER_SIZE];
//Transmit DMA descriptors
#pragma data_alignment = 8
#pragma location = SC589_ETH2_RAM_SECTION
static Sc589Eth2TxDmaDesc txDmaDesc[SC589_ETH2_TX_BUFFER_COUNT];
//Receive DMA descriptors
#pragma data_alignment = 8
#pragma location = SC589_ETH2_RAM_SECTION
static Sc589Eth2RxDmaDesc rxDmaDesc[SC589_ETH2_RX_BUFFER_COUNT];
...
//Transmit buffer
static uint8_t txBuffer[SC589_ETH2_TX_BUFFER_COUNT][SC589_ETH2_TX_BUFFER_SIZE]
   __attribute__((aligned(4), __section__(SC589_ETH2_RAM_SECTION)));
//Receive buffer
static uint8_t rxBuffer[SC589_ETH2_RX_BUFFER_COUNT][SC589_ETH2_RX_BUFFER_SIZE]
   __attribute__((aligned(4), __section__(SC589_ETH2_RAM_SECTION)));
//Transmit DMA descriptors
static Sc589Eth2TxDmaDesc txDmaDesc[SC589_ETH2_TX_BUFFER_COUNT]
   __attribute__((aligned(8), __section__(SC589_ETH2_RAM_SECTION)));
//Receive DMA descriptors
static Sc589Eth2RxDmaDesc rxDmaDesc[SC589_ETH2_RX_BUFFER_COUNT]
   __attribute__((aligned(8), __section__(SC589_ETH2_RAM_SECTION)));
...
static Sc589Eth2TxDmaDesc *txCurDmaDesc;
//Pointer to the current RX DMA descriptor
static Sc589Eth2RxDmaDesc *rxCurDmaDesc;
...
const NicDriver sc589Eth2Driver =
{
   ...
   sc589Eth2EnableIrq,
   sc589Eth2DisableIrq,
   sc589Eth2EventHandler,
   sc589Eth2SendPacket,
   sc589Eth2UpdateMacAddrFilter,
   sc589Eth2UpdateMacConfig,
   sc589Eth2WritePhyReg,
   sc589Eth2ReadPhyReg,
   TRUE,
   TRUE,
   TRUE,
...
error_t sc589Eth2Init(NetInterface *interface)
{
   ...
   //Debug message
   TRACE_INFO("Initializing ADSP-SC589 Ethernet MAC (EMAC1)...\r\n");

   //Save underlying network interface
   nicDriverInterface = interface;

   //GPIO configuration
   sc589Eth2InitGpio(interface);

   //Perform a software reset
   *pREG_EMAC1_DMA0_BUSMODE |= BITM_EMAC_DMA0_BUSMODE_SWR;
   //Wait for the reset to complete
   while((*pREG_EMAC1_DMA0_BUSMODE & BITM_EMAC_DMA0_BUSMODE_SWR) != 0)
   {
   }

   //Adjust MDC clock range depending on CLKO7 frequency
   *pREG_EMAC1_SMI_ADDR = ENUM_EMAC_SMI_ADDR_CR_DIV62;
   ...
   *pREG_EMAC1_MACCFG = BITM_EMAC_MACCFG_PS | BITM_EMAC_MACCFG_DO;

   //Configure MAC address filtering
   sc589Eth2UpdateMacAddrFilter(interface);

   //Disable flow control
   *pREG_EMAC1_FLOWCTL = 0;

   //Set the threshold level of the transmit and receive FIFOs
   *pREG_EMAC1_DMA0_OPMODE = ENUM_EMAC_DMA1_OPMODE_TTC_64 |
      ENUM_EMAC_DMA1_OPMODE_RTC_32;

   //Configure DMA bus mode
   *pREG_EMAC1_DMA0_BUSMODE = BITM_EMAC_DMA0_BUSMODE_AAL |
      BITM_EMAC_DMA0_BUSMODE_USP | ENUM_EMAC_DMA_BUSMODE_RPBL_32 |
      ENUM_EMAC_DMA_BUSMODE_PBL_32;

   //Initialize DMA descriptor lists
   sc589Eth2InitDmaDesc(interface);

   //Prevent interrupts from being generated when statistic counters reach
   //half their maximum value
   *pREG_EMAC1_MMC_TXIMSK = 0x01FFFFFF;
   *pREG_EMAC1_MMC_RXIMSK = 0x01FFFFFF;
   *pREG_EMAC1_IPC_RXIMSK = 0x3FFFFFFF;

   //Disable MAC interrupts
   *pREG_EMAC1_IMSK = BITM_EMAC_IMSK_LPIIM | BITM_EMAC_IMSK_TS;

   //Enable the desired DMA interrupts
   *pREG_EMAC1_DMA0_IEN = BITM_EMAC_DMA0_IEN_NIE | BITM_EMAC_DMA0_IEN_RIE |
      BITM_EMAC_DMA0_IEN_TIE;

   //Register interrupt handler
   adi_int_InstallHandler(INTR_EMAC1_STAT, sc589Eth2IrqHandler, interface,
      false);

   //Enable MAC transmission and reception
   *pREG_EMAC1_MACCFG |= BITM_EMAC_MACCFG_TE | BITM_EMAC_MACCFG_RE;

   //Enable DMA transmission and reception
   *pREG_EMAC1_DMA0_OPMODE |= BITM_EMAC_DMA0_OPMODE_ST |
      BITM_EMAC_DMA0_OPMODE_SR;
...
//ADZS-SC589-EZLITE evaluation board?
   ...
   //Configure PF_13 (ETH1_CRS), PF_14 (ETH1_MDC) and PF_15 (ETH1_MDIO)
   temp = *pREG_PORTF_MUX;
   ...
   *pREG_PORTF_FER_SET = BITM_PORT_FER_PX13 | BITM_PORT_FER_PX14 |
      BITM_PORT_FER_PX15;

   //Configure PG_00 (ETH1_REFCLK), PG_01 (ETH1_TXEN), PG_02 (ETH1_TXD0),
   //PG_03 (ETH1_TXD1) and PG_04 (ETH1_RXD0) and PG_05 (ETH1_RXD1)
   temp = *pREG_PORTG_MUX;
   ...
   *pREG_PORTG_MUX = temp;
   ...
   //Select peripheral mode
   *pREG_PORTG_FER_SET = BITM_PORT_FER_PX0 | BITM_PORT_FER_PX1 |
      BITM_PORT_FER_PX2 | BITM_PORT_FER_PX3 | BITM_PORT_FER_PX4 |
      BITM_PORT_FER_PX5;

   //Configure ETH1_INT (PD_00) as an input
   *pREG_PORTD_FER_CLR = BITM_PORT_FER_PX0;
   *pREG_PORTD_DIR_CLR = BITM_PORT_DIR_PX0;
   *pREG_PORTD_INEN_SET = BITM_PORT_INEN_PX0;
#endif
}
...
void sc589Eth2InitDmaDesc(NetInterface *interface)
{
   ...
   for(i = 0; i < SC589_ETH2_TX_BUFFER_COUNT; i++)
   {
   ...
   for(i = 0; i < SC589_ETH2_RX_BUFFER_COUNT; i++)
   {
      //The descriptor is initially owned by the DMA
      rxDmaDesc[i].rdes0 = EMAC_RDES0_OWN;
      //Use chain structure rather than ring structure
      rxDmaDesc[i].rdes1 = EMAC_RDES1_RCH | (SC589_ETH2_RX_BUFFER_SIZE & EMAC_RDES1_RBS1);
...
void sc589Eth2IrqHandler(uint32_t id, void *param)
{
   ...
   if((status & BITM_EMAC_DMA0_STAT_TI) != 0)
   {
      ...
      *pREG_EMAC1_DMA0_STAT = BITM_EMAC_DMA0_STAT_TI;
   ...
   if((status & BITM_EMAC_DMA0_STAT_RI) != 0)
   {
      ...
      *pREG_EMAC1_DMA0_STAT = BITM_EMAC_DMA0_STAT_RI;
   ...
   *pREG_EMAC1_DMA0_STAT = BITM_EMAC_DMA0_STAT_NIS;
...
void sc589Eth2EventHandler(NetInterface *interface)
{
   ...
   do
   {
      ...
      error = sc589Eth2ReceivePacket(interface);
...
error_t sc589Eth2SendPacket(NetInterface *interface,
   const NetBuffer *buffer, size_t offset, NetTxAncillary *ancillary)
{
   ...
   if(length > SC589_ETH2_TX_BUFFER_SIZE)
   {
   ...
   *pREG_EMAC1_DMA0_STAT = BITM_EMAC_DMA0_STAT_TU;
   //Instruct the DMA to poll the transmit descriptor list
   *pREG_EMAC1_DMA0_TXPOLL = 0;

   //Point to the next descriptor in the list
   txCurDmaDesc = (Sc589Eth2TxDmaDesc *) adi_rtl_system_to_internal_addr(
      txCurDmaDesc->tdes3);
...
error_t sc589Eth2ReceivePacket(NetInterface *interface)
{
   ...
   if((rxCurDmaDesc->rdes0 & EMAC_RDES0_OWN) == 0)
   {
         ...
         (rxCurDmaDesc->rdes0 & EMAC_RDES0_LS) != 0)
      {
         ...
         if((rxCurDmaDesc->rdes0 & EMAC_RDES0_ES) == 0)
         {
            ...
            n = MIN(n, SC589_ETH2_RX_BUFFER_SIZE);
      ...
      //Point to the next descriptor in the list
      rxCurDmaDesc = (Sc589Eth2RxDmaDesc *) adi_rtl_system_to_internal_addr(
         rxCurDmaDesc->rdes3);
   }
   else
   {
   ...
   *pREG_EMAC1_DMA0_STAT = BITM_EMAC_DMA0_STAT_RU;
   //Instruct the DMA to poll the receive descriptor list
   *pREG_EMAC1_DMA0_RXPOLL = 0;
...
error_t sc589Eth2UpdateMacAddrFilter(NetInterface *interface)
{
   ...
   if(interface->promiscuous)
   {
      ...
      *pREG_EMAC1_MACFRMFILT = BITM_EMAC_MACFRMFILT_PR;
   }
   else
   {
      ...
      *pREG_EMAC1_ADDR0_HI = interface->macAddr.w[2];
      ...
      for(i = 0, j = 0; i < MAC_ADDR_FILTER_SIZE; i++)
      {
         ...
         if(entry->refCount > 0)
         {
            ...
            if(macIsMulticastAddr(&entry->addr))
            {
               ...
               crc = sc589Eth2CalcCrc(&entry->addr, sizeof(MacAddr));
      ...
      if(j >= 1)
      {
         ...
         *pREG_EMAC1_ADDR1_HI = unicastMacAddr[0].w[2] | BITM_EMAC_ADDR1_HI_AE;
      }
      else
      {
      ...
      if(interface->acceptAllMulticast)
      {
         ...
         *pREG_EMAC1_MACFRMFILT = BITM_EMAC_MACFRMFILT_HPF | BITM_EMAC_MACFRMFILT_PM;
      }
      else
      {
         //Configure the receive filter
         *pREG_EMAC1_MACFRMFILT = BITM_EMAC_MACFRMFILT_HPF | BITM_EMAC_MACFRMFILT_HMC;

         //Configure the multicast hash table
         *pREG_EMAC1_HASHTBL_LO = hashTable[0];
         *pREG_EMAC1_HASHTBL_HI = hashTable[1];
...
void sc589Eth2WritePhyReg(uint8_t opcode, uint8_t phyAddr,
   uint8_t regAddr, uint16_t data)
{
   ...
   if(opcode == SMI_OPCODE_WRITE)
   {
      ...
      temp = *pREG_EMAC1_SMI_ADDR & BITM_EMAC_SMI_ADDR_CR;
      ...
      *pREG_EMAC1_SMI_DATA = data & BITM_EMAC_SMI_DATA_SMID;

      //Start a write operation
      *pREG_EMAC1_SMI_ADDR = temp;
      //Wait for the write to complete
      while((*pREG_EMAC1_SMI_ADDR & BITM_EMAC_SMI_ADDR_SMIB) != 0)
      {
      }
   }
   else
...
uint16_t sc589Eth2ReadPhyReg(uint8_t opcode, uint8_t phyAddr,
   uint8_t regAddr)
{
   uint16_t data;
   uint32_t temp;
   ...
   if(opcode == SMI_OPCODE_READ)
   {
      ...
      *pREG_EMAC1_SMI_ADDR = temp;
      ...
      //Get register value
      data = *pREG_EMAC1_SMI_DATA & BITM_EMAC_SMI_DATA_SMID;
   }
   else
   {
      //The MAC peripheral only supports standard Clause 22 opcodes
