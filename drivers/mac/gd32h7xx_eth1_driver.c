...
#include "gd32h7xx.h"
#include "core/net.h"
#include "drivers/mac/gd32h7xx_eth1_driver.h"
#include "debug.h"
...
#pragma location = STM32H7XX_ETH1_RAM_SECTION
static uint8_t txBuffer[GD32H7XX_ETH1_TX_BUFFER_COUNT][GD32H7XX_ETH1_TX_BUFFER_SIZE];
//Receive buffer
#pragma data_alignment = 4
#pragma location = STM32H7XX_ETH1_RAM_SECTION
static uint8_t rxBuffer[GD32H7XX_ETH1_RX_BUFFER_COUNT][GD32H7XX_ETH1_RX_BUFFER_SIZE];
//Transmit DMA descriptors
#pragma data_alignment = 4
#pragma location = STM32H7XX_ETH1_RAM_SECTION
static Gd32h7xxEth1TxDmaDesc txDmaDesc[GD32H7XX_ETH1_TX_BUFFER_COUNT];
//Receive DMA descriptors
#pragma data_alignment = 4
#pragma location = STM32H7XX_ETH1_RAM_SECTION
static Gd32h7xxEth1RxDmaDesc rxDmaDesc[GD32H7XX_ETH1_RX_BUFFER_COUNT];
...
//Transmit buffer
static uint8_t txBuffer[GD32H7XX_ETH1_TX_BUFFER_COUNT][GD32H7XX_ETH1_TX_BUFFER_SIZE]
   __attribute__((aligned(4), __section__(STM32H7XX_ETH1_RAM_SECTION)));
//Receive buffer
static uint8_t rxBuffer[GD32H7XX_ETH1_RX_BUFFER_COUNT][GD32H7XX_ETH1_RX_BUFFER_SIZE]
   __attribute__((aligned(4), __section__(STM32H7XX_ETH1_RAM_SECTION)));
//Transmit DMA descriptors
...
static Gd32h7xxEth1TxDmaDesc *txCurDmaDesc;
//Pointer to the current RX DMA descriptor
static Gd32h7xxEth1RxDmaDesc *rxCurDmaDesc;


/**
 * @brief GD32H7 Ethernet MAC driver (ENET0 instance)
 **/
...
const NicDriver gd32h7xxEth1Driver =
{
   ...
   gd32h7xxEth1Init,
   gd32h7xxEth1Tick,
   gd32h7xxEth1EnableIrq,
   gd32h7xxEth1DisableIrq,
   gd32h7xxEth1EventHandler,
   gd32h7xxEth1SendPacket,
   gd32h7xxEth1UpdateMacAddrFilter,
   gd32h7xxEth1UpdateMacConfig,
   gd32h7xxEth1WritePhyReg,
   gd32h7xxEth1ReadPhyReg,
   TRUE,
   TRUE,
   TRUE,
   FALSE
...
error_t gd32h7xxEth1Init(NetInterface *interface)
{
   ...
   //Debug message
   TRACE_INFO("Initializing GD32H7 Ethernet MAC (ENET0)...\r\n");

   //Save underlying network interface
   nicDriverInterface = interface;

   //GPIO configuration
   gd32h7xxEth1InitGpio(interface);
   ...
   ENET_DMA_BCTL(ENET0) |= ENET_DMA_BCTL_SWR;
   //Wait for the reset to complete
   while((ENET_DMA_BCTL(ENET0) & ENET_DMA_BCTL_SWR) != 0)
   {
   }

   //Adjust MDC clock range depending on HCLK frequency
   ENET_MAC_PHY_CTL(ENET0) = ENET_MDC_HCLK_DIV62;
   ...
   ENET_MAC_CFG(ENET0) = ENET_MAC_CFG_ROD;

   //Set the MAC address of the station
   ENET_MAC_ADDR0L(ENET0) = interface->macAddr.w[0] | (interface->macAddr.w[1] << 16);
   ENET_MAC_ADDR0H(ENET0) = interface->macAddr.w[2] | ENET_MAC_ADDR0H_MO;

   //The MAC supports 3 additional addresses for unicast perfect filtering
   ENET_MAC_ADDR1L(ENET0) = 0;
   ENET_MAC_ADDR1H(ENET0) = 0;
   ENET_MAC_ADDR2L(ENET0) = 0;
   ENET_MAC_ADDT2H(ENET0) = 0;
   ENET_MAC_ADDR3L(ENET0) = 0;
   ENET_MAC_ADDR3H(ENET0) = 0;

   //Initialize hash table
   ENET_MAC_HLL(ENET0) = 0;
   ENET_MAC_HLH(ENET0) = 0;

   //Configure the receive filter
   ENET_MAC_FRMF(ENET0) = ENET_MAC_FRMF_HPFLT | ENET_MAC_FRMF_HMF;
   //Disable flow control
   ENET_MAC_FCTL(ENET0) = 0;
   //Enable store and forward mode
   ENET_DMA_CTL(ENET0) = ENET_DMA_CTL_RSFD | ENET_DMA_CTL_TSFD;

   //Configure DMA bus mode
   ENET_DMA_BCTL(ENET0) = ENET_DMA_BCTL_AA | ENET_DMA_BCTL_UIP | ENET_RXDP_32BEAT |
      ENET_ARBITRATION_RXTX_1_1 | ENET_PGBL_32BEAT | ENET_DMA_BCTL_DFM;

   //Initialize DMA descriptor lists
   gd32h7xxEth1InitDmaDesc(interface);

   //Prevent interrupts from being generated when the transmit statistic
   //counters reach half their maximum value
   ENET_MSC_TINTMSK(ENET0) = ENET_MSC_TINTMSK_TGFIM | ENET_MSC_TINTMSK_TGFMSCIM |
      ENET_MSC_TINTMSK_TGFSCIM;

   //Prevent interrupts from being generated when the receive statistic
   //counters reach half their maximum value
   ENET_MSC_RINTMSK(ENET0) = ENET_MSC_RINTMSK_RGUFIM | ENET_MSC_RINTMSK_RFAEIM |
      ENET_MSC_RINTMSK_RFCEIM;

   //Disable MAC interrupts
   ENET_MAC_INTMSK(ENET0) = ENET_MAC_INTMSK_TMSTIM | ENET_MAC_INTMSK_WUMIM;
   //Enable the desired DMA interrupts
   ENET_DMA_INTEN(ENET0) = ENET_DMA_INTEN_NIE | ENET_DMA_INTEN_RIE | ENET_DMA_INTEN_TIE;

   //Set priority grouping (4 bits for pre-emption priority, no bits for subpriority)
   NVIC_SetPriorityGrouping(GD32H7XX_ETH1_IRQ_PRIORITY_GROUPING);

   //Configure Ethernet interrupt priority
   NVIC_SetPriority(ENET0_IRQn, NVIC_EncodePriority(GD32H7XX_ETH1_IRQ_PRIORITY_GROUPING,
      GD32H7XX_ETH1_IRQ_GROUP_PRIORITY, GD32H7XX_ETH1_IRQ_SUB_PRIORITY));

   //Enable MAC transmission and reception
   ENET_MAC_CFG(ENET0) |= ENET_MAC_CFG_TEN | ENET_MAC_CFG_REN;
   //Enable DMA transmission and reception
   ENET_DMA_CTL(ENET0) |= ENET_DMA_CTL_STE | ENET_DMA_CTL_SRE;
...
{
//GD32H759I-EVAL evaluation board?
#if defined(USE_GD32H759I_EVAL)
   //Enable SYSCFG clock
   ...
   gpio_output_options_set(GPIOA, GPIO_OTYPE_PP, GPIO_OSPEED_12MHZ, GPIO_PIN_2);
   ...
   gpio_output_options_set(GPIOC, GPIO_OTYPE_PP, GPIO_OSPEED_12MHZ, GPIO_PIN_1);
...
void gd32h7xxEth1InitDmaDesc(NetInterface *interface)
{
   ...
   for(i = 0; i < GD32H7XX_ETH1_TX_BUFFER_COUNT; i++)
   {
   ...
   for(i = 0; i < GD32H7XX_ETH1_RX_BUFFER_COUNT; i++)
   {
      //The descriptor is initially owned by the DMA
      rxDmaDesc[i].rdes0 = ENET_RDES0_DAV;
      //Use chain structure rather than ring structure
      rxDmaDesc[i].rdes1 = ENET_RDES1_RCHM | (GD32H7XX_ETH1_RX_BUFFER_SIZE & ENET_RDES1_RB1S);
   ...
   ENET_DMA_TDTADDR(ENET0) = (uint32_t) txDmaDesc;
   //Start location of the RX descriptor list
   ENET_DMA_RDTADDR(ENET0) = (uint32_t) rxDmaDesc;
}
...
void ENET0_IRQHandler(void)
{
   ...
   status = ENET_DMA_STAT(ENET0);

   //Packet transmitted?
   if((status & ENET_DMA_STAT_TS) != 0)
   {
      //Clear TS interrupt flag
      ENET_DMA_STAT(ENET0) = ENET_DMA_STAT_TS;
   ...
   if((status & ENET_DMA_STAT_RS) != 0)
   {
      ...
      ENET_DMA_STAT(ENET0) = ENET_DMA_STAT_RS;
   ...
   ENET_DMA_STAT(ENET0) = ENET_DMA_STAT_NI;
...
void gd32h7xxEth1EventHandler(NetInterface *interface)
{
   ...
   do
   {
      ...
      error = gd32h7xxEth1ReceivePacket(interface);
   ...
   const NetBuffer *buffer, size_t offset, NetTxAncillary *ancillary)
{
   ...
   if(length > GD32H7XX_ETH1_TX_BUFFER_SIZE)
   {
   ...
   ENET_DMA_STAT(ENET0) = ENET_DMA_STAT_TBU;
   //Instruct the DMA to poll the transmit descriptor list
   ENET_DMA_TPEN(ENET0) = 0;

   //Point to the next descriptor in the list
   txCurDmaDesc = (Gd32h7xxEth1TxDmaDesc *) txCurDmaDesc->tdes3;
...
error_t gd32h7xxEth1ReceivePacket(NetInterface *interface)
{
   ...
   if((rxCurDmaDesc->rdes0 & ENET_RDES0_DAV) == 0)
   {
         ...
         (rxCurDmaDesc->rdes0 & ENET_RDES0_LDES) != 0)
      {
         ...
         if((rxCurDmaDesc->rdes0 & ENET_RDES0_ERRS) == 0)
         {
            ...
            n = MIN(n, GD32H7XX_ETH1_RX_BUFFER_SIZE);
      ...
      //Point to the next descriptor in the list
      rxCurDmaDesc = (Gd32h7xxEth1RxDmaDesc *) rxCurDmaDesc->rdes3;
   }
   else
   {
      //No more data in the receive buffer
   ...
   ENET_DMA_STAT(ENET0) = ENET_DMA_STAT_RBU;
   //Instruct the DMA to poll the receive descriptor list
   ENET_DMA_RPEN(ENET0) = 0;
...
error_t gd32h7xxEth1UpdateMacAddrFilter(NetInterface *interface)
{
   ...
   for(i = 0, j = 0; i < MAC_ADDR_FILTER_SIZE; i++)
   {
      ...
      if(entry->refCount > 0)
      {
         ...
         if(macIsMulticastAddr(&entry->addr))
         {
            ...
            crc = gd32h7xxEth1CalcCrc(&entry->addr, sizeof(MacAddr));
   ...
   if(j >= 1)
   {
      ...
      ENET_MAC_ADDR1L(ENET0) = unicastMacAddr[0].w[0] | (unicastMacAddr[0].w[1] << 16);
      ENET_MAC_ADDR1H(ENET0) = unicastMacAddr[0].w[2] | ENET_MAC_ADDR1H_AFE;
   }
   else
   {
   ...
   {
      //When the AE bit is set, the entry is used for perfect filtering
      ENET_MAC_ADDR2L(ENET0) = unicastMacAddr[1].w[0] | (unicastMacAddr[1].w[1] << 16);
      ENET_MAC_ADDT2H(ENET0) = unicastMacAddr[1].w[2] | ENET_MAC_ADDR2H_AFE;
   }
   else
   {
      //When the AE bit is cleared, the entry is ignored
   ...
   {
      //When the AE bit is set, the entry is used for perfect filtering
      ENET_MAC_ADDR3L(ENET0) = unicastMacAddr[2].w[0] | (unicastMacAddr[2].w[1] << 16);
      ENET_MAC_ADDR3H(ENET0) = unicastMacAddr[2].w[2] | ENET_MAC_ADDR3H_AFE;
   }
   else
   {
      //When the AE bit is cleared, the entry is ignored
   ...
   ENET_MAC_HLL(ENET0) = hashTable[0];
   ENET_MAC_HLH(ENET0) = hashTable[1];

   //Debug message
   TRACE_DEBUG("  ENET_MAC_HLL = 0x%08" PRIX32 "\r\n", ENET_MAC_HLL(ENET0));
   TRACE_DEBUG("  ENET_MAC_HLH = 0x%08" PRIX32 "\r\n", ENET_MAC_HLH(ENET0));
   ...
   uint8_t regAddr, uint16_t data)
{
   ...
   if(opcode == SMI_OPCODE_WRITE)
   {
      ...
      temp = ENET_MAC_PHY_CTL(ENET0) & ENET_MAC_PHY_CTL_CLR;
      ...
      ENET_MAC_PHY_DATA(ENET0) = data & ENET_MAC_PHY_DATA_PD;

      //Start a write operation
      ENET_MAC_PHY_CTL(ENET0) = temp;
      //Wait for the write to complete
      while((ENET_MAC_PHY_CTL(ENET0) & ENET_MAC_PHY_CTL_PB) != 0)
      {
      }
   }
   else
   ...
   uint8_t regAddr)
{
   ...
   if(opcode == SMI_OPCODE_READ)
   {
      ...
      //Get register value
      data = ENET_MAC_PHY_DATA(ENET0) & ENET_MAC_PHY_DATA_PD;
   }
   else
   {
      //The MAC peripheral only supports standard Clause 22 opcodes
