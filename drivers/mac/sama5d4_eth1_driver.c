/**
 * @file sama5d4_eth1_driver.c
 * @brief SAMA5D4 Ethernet MAC driver (GMAC0 instance)
 *
 * @section License
 *
...
#include "barriers.h"
#include "core/net.h"
#include "drivers/mac/sama5d4_eth1_driver.h"
#include "debug.h"
...
#pragma location = SAMA5D4_ETH1_RAM_SECTION
static uint8_t txBuffer[SAMA5D4_ETH1_TX_BUFFER_COUNT][SAMA5D4_ETH1_TX_BUFFER_SIZE];
//RX buffer
#pragma data_alignment = 8
#pragma location = SAMA5D4_ETH1_RAM_SECTION
static uint8_t rxBuffer[SAMA5D4_ETH1_RX_BUFFER_COUNT][SAMA5D4_ETH1_RX_BUFFER_SIZE];
//TX buffer descriptors
#pragma data_alignment = 8
#pragma location = SAMA5D4_ETH1_RAM_SECTION
static Sama5d4Eth1TxBufferDesc txBufferDesc[SAMA5D4_ETH1_TX_BUFFER_COUNT];
//RX buffer descriptors
#pragma data_alignment = 8
#pragma location = SAMA5D4_ETH1_RAM_SECTION
static Sama5d4Eth1RxBufferDesc rxBufferDesc[SAMA5D4_ETH1_RX_BUFFER_COUNT];
...
//TX buffer
static uint8_t txBuffer[SAMA5D4_ETH1_TX_BUFFER_COUNT][SAMA5D4_ETH1_TX_BUFFER_SIZE]
   __attribute__((aligned(8), __section__(SAMA5D4_ETH1_RAM_SECTION)));
//RX buffer
static uint8_t rxBuffer[SAMA5D4_ETH1_RX_BUFFER_COUNT][SAMA5D4_ETH1_RX_BUFFER_SIZE]
   __attribute__((aligned(8), __section__(SAMA5D4_ETH1_RAM_SECTION)));
//TX buffer descriptors
...
error_t sama5d4Eth1Init(NetInterface *interface)
{
   ...
   //Debug message
   TRACE_INFO("Initializing SAMA5D4 Ethernet MAC (GMAC0)...\r\n");
   ...
   sama5d4Eth1InitBufferDesc(interface);
   ...
   AIC->AIC_SMR = AIC_SMR_SRCTYPE_INT_LEVEL_SENSITIVE | AIC_SMR_PRIOR(SAMA5D4_ETH1_IRQ_PRIORITY);
   AIC->AIC_SVR = (uint32_t) sama5d4Eth1IrqHandler;
...
{
//SAMA5D4-Xplained-Ultra evaluation board?
   ...
   //Configure RMII pins
   mask = PIO_PB0A_G0_TXCK | PIO_PB2A_G0_TXEN | PIO_PB6A_G0_RXDV |
     PIO_PB7A_G0_RXER | PIO_PB8A_G0_RX0 | PIO_PB9A_G0_RX1 | PIO_PB12A_G0_TX0 |
     PIO_PB13A_G0_TX1 | PIO_PB16A_G0_MDC | PIO_PB17A_G0_MDIO;
...
void sama5d4Eth1InitBufferDesc(NetInterface *interface)
{
   ...
   for(i = 0; i < SAMA5D4_ETH1_TX_BUFFER_COUNT; i++)
   {
   ...
   for(i = 0; i < SAMA5D4_ETH1_RX_BUFFER_COUNT; i++)
   {
...
void sama5d4Eth1EventHandler(NetInterface *interface)
{
   ...
   if((rsr & (GMAC_RSR_HNO | GMAC_RSR_RXOVR | GMAC_RSR_REC | GMAC_RSR_BNA)) != 0)
   {
      ...
      do
      {
         ...
         error = sama5d4Eth1ReceivePacket(interface);
   ...
   const NetBuffer *buffer, size_t offset, NetTxAncillary *ancillary)
{
   ...
   if(length > SAMA5D4_ETH1_TX_BUFFER_SIZE)
   {
   ...
   if(txBufferIndex < (SAMA5D4_ETH1_TX_BUFFER_COUNT - 1))
   {
...
error_t sama5d4Eth1ReceivePacket(NetInterface *interface)
{
   ...
   for(i = 0; i < SAMA5D4_ETH1_RX_BUFFER_COUNT; i++)
   {
      ...
      if(j >= SAMA5D4_ETH1_RX_BUFFER_COUNT)
      {
         j -= SAMA5D4_ETH1_RX_BUFFER_COUNT;
      }
   ...
   for(i = 0; i < j; i++)
   {
      ...
      if(eofIndex != UINT_MAX && i >= sofIndex && i <= eofIndex)
      {
         ...
         n = MIN(size, SAMA5D4_ETH1_RX_BUFFER_SIZE);
      ...
      //Wrap around to the beginning of the buffer if necessary
      if(rxBufferIndex >= SAMA5D4_ETH1_RX_BUFFER_COUNT)
      {
         rxBufferIndex = 0;
      }
   }
...
void sama5d4Eth1WritePhyReg(uint8_t opcode, uint8_t phyAddr,
...
uint16_t sama5d4Eth1ReadPhyReg(uint8_t opcode, uint8_t phyAddr,
   uint8_t regAddr)
{
   uint16_t data;
   uint32_t temp;
