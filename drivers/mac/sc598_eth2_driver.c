/**
 * @file sc598_eth2_driver.c
 * @brief ADSP-SC598 Ethernet MAC driver (EMAC1 instance)
 *
 * @section License
 *
...
#include "core/net.h"
#include "drivers/mac/sc598_eth2_driver.h"
#include "debug.h"
...
#pragma location = SC598_ETH2_RAM_SECTION
static uint8_t txBuffer[SC598_ETH2_TX_BUFFER_COUNT][SC598_ETH2_TX_BUFFER_SIZE];
//Receive buffer
#pragma data_alignment = 4
#pragma location = SC598_ETH2_RAM_SECTION
static uint8_t rxBuffer[SC598_ETH2_RX_BUFFER_COUNT][SC598_ETH2_RX_BUFFER_SIZE];
//Transmit DMA descriptors
#pragma data_alignment = 8
#pragma location = SC598_ETH2_RAM_SECTION
static Sc598Eth2TxDmaDesc txDmaDesc[SC598_ETH2_TX_BUFFER_COUNT];
//Receive DMA descriptors
#pragma data_alignment = 8
#pragma location = SC598_ETH2_RAM_SECTION
static Sc598Eth2RxDmaDesc rxDmaDesc[SC598_ETH2_RX_BUFFER_COUNT];
...
//Transmit buffer
static uint8_t txBuffer[SC598_ETH2_TX_BUFFER_COUNT][SC598_ETH2_TX_BUFFER_SIZE]
   __attribute__((aligned(4), __section__(SC598_ETH2_RAM_SECTION)));
//Receive buffer
static uint8_t rxBuffer[SC598_ETH2_RX_BUFFER_COUNT][SC598_ETH2_RX_BUFFER_SIZE]
   __attribute__((aligned(4), __section__(SC598_ETH2_RAM_SECTION)));
//Transmit DMA descriptors
static Sc598Eth2TxDmaDesc txDmaDesc[SC598_ETH2_TX_BUFFER_COUNT]
   __attribute__((aligned(8), __section__(SC598_ETH2_RAM_SECTION)));
//Receive DMA descriptors
static Sc598Eth2RxDmaDesc rxDmaDesc[SC598_ETH2_RX_BUFFER_COUNT]
   __attribute__((aligned(8), __section__(SC598_ETH2_RAM_SECTION)));
...
const NicDriver sc598Eth2Driver =
{
   ...
   sc598Eth2EnableIrq,
   sc598Eth2DisableIrq,
   sc598Eth2EventHandler,
   sc598Eth2SendPacket,
   sc598Eth2UpdateMacAddrFilter,
   sc598Eth2UpdateMacConfig,
   sc598Eth2WritePhyReg,
   sc598Eth2ReadPhyReg,
   TRUE,
   TRUE,
   TRUE,
...
error_t sc598Eth2Init(NetInterface *interface)
{
   ...
   //Debug message
   TRACE_INFO("Initializing ADSP-SC598 Ethernet MAC (EMAC1)...\r\n");

   //Save underlying network interface
   nicDriverInterface = interface;

   //GPIO configuration
   sc598Eth2InitGpio(interface);

   //Set DMA transfer format (little-endian)
   *pREG_PADS0_PCFG0 &= ~BITM_PADS_PCFG0_EMAC1_ENDIANNESS;

   //Perform a software reset
   *pREG_EMAC1_DMA_MODE |= BITM_EMAC_DMA_MODE_SWR;
   //Wait for the reset to complete
   while((*pREG_EMAC1_DMA_MODE & BITM_EMAC_DMA_MODE_SWR) != 0)
   {
   }

   //Adjust MDC clock range depending on SCLK0 frequency
   *pREG_EMAC1_MDIO_ADDR = (4 << BITP_EMAC_MDIO_ADDR_CR);
   ...
   *pREG_EMAC1_MAC_CFG = BITM_EMAC_MAC_CFG_GPSLCE | BITM_EMAC_MAC_CFG_PS |
      BITM_EMAC_MAC_CFG_DO;

   //Set the maximum packet size that can be accepted
   temp = *pREG_EMAC1_MAC_EXT_CFG & ~BITM_EMAC_MAC_EXT_CFG_GPSL;
   *pREG_EMAC1_MAC_EXT_CFG = temp | SC598_ETH2_RX_BUFFER_SIZE;

   //Configure MAC address filtering
   sc598Eth2UpdateMacAddrFilter(interface);

   //Disable flow control
   *pREG_EMAC1_Q0_TXFLOW_CTL = 0;
   *pREG_EMAC1_RXFLOW_CTL = 0;

   //Configure DMA operating mode
   *pREG_EMAC1_DMA_MODE = ENUM_EMAC_DMA_MODE_MODE0 |
      ENUM_EMAC_DMA_MODE_DSPW_DISABLE;

   //Configure system bus mode
   *pREG_EMAC1_DMA_SYSBMODE |= BITM_EMAC_DMA_SYSBMODE_AAL;

   //The DMA takes the descriptor table as contiguous
   *pREG_EMAC1_DMA0_CTL = (0 << BITP_EMAC_DMA_CTL_DSL);
   //Configure TX features
   *pREG_EMAC1_DMA0_TXCTL = (32 << BITP_EMAC_DMA_TXCTL_TXPBL);

   //Configure RX features
   *pREG_EMAC1_DMA0_RXCTL = (32 << BITP_EMAC_DMA_RXCTL_RXPBL) |
      ((SC598_ETH2_RX_BUFFER_SIZE / 4) << BITP_EMAC_DMA_RXCTL_RBSZ_13_Y);

   //Set the threshold level of the MTL TX queue
   *pREG_EMAC1_TQ0_OPMODE = (1 << BITP_EMAC_TQ_OPMODE_TTC);
   //Set the threshold level of the MTL RX queue
   *pREG_EMAC1_RQ0_OPMODE = (1 << BITP_EMAC_RQ_OPMODE_RTC);

   //Initialize DMA descriptor lists
   sc598Eth2InitDmaDesc(interface);

   //Prevent interrupts from being generated when statistic counters reach
   //half their maximum value
   *pREG_EMAC1_MMC_TXIMSK = 0x0FFFFFFF;
   *pREG_EMAC1_MMC_RXIMSK = 0x0FFFFFFF;
   *pREG_EMAC1_MMC_IPC_RXIMSK = 0x3FFFFFFF;

   //Disable MAC interrupts
   *pREG_EMAC1_MAC_IEN = 0;

   //Enable the desired DMA interrupts
   *pREG_EMAC1_DMA0_IEN = BITM_EMAC_DMA_IEN_NIE | BITM_EMAC_DMA_IEN_RIE |
      BITM_EMAC_DMA_IEN_TIE;

   //Register interrupt handler
   adi_int_InstallHandler(INTR_EMAC1_STAT, sc598Eth2IrqHandler, interface,
      false);

   //Enable MAC transmission and reception
   *pREG_EMAC1_MAC_CFG |= BITM_EMAC_MAC_CFG_TE | BITM_EMAC_MAC_CFG_RE;

   //Enable DMA transmission and reception
   *pREG_EMAC1_DMA0_TXCTL |= BITM_EMAC_DMA_TXCTL_ST;
   *pREG_EMAC1_DMA0_RXCTL |= BITM_EMAC_DMA_RXCTL_SR;
...
//EV-SC598-SOM evaluation board?
   ...
   //Configure PE_11 (ETH1_REFCLK), PE_12 (ETH1_TXEN), PE_13 (ETH1_TXD0),
   //PE_14 (ETH1_TXD1) and PE_15 (ETH1_RXD0)
   temp = *pREG_PORTE_MUX;
   ...
   *pREG_PORTE_FER_SET = BITM_PORT_FER_PX11 | BITM_PORT_FER_PX12 |
      BITM_PORT_FER_PX13 | BITM_PORT_FER_PX14 | BITM_PORT_FER_PX15;

   //Configure PF_00 (ETH1_RXD1), PF_01 (ETH1_MDIO), PF_02 (ETH1_MDC) and
   //PF_03 (ETH1_CRS)
   temp = *pREG_PORTF_MUX;
   ...
   *pREG_PORTF_MUX = temp;
   ...
   //Select peripheral mode
   *pREG_PORTF_FER_SET = BITM_PORT_FER_PX0 | BITM_PORT_FER_PX1 |
      BITM_PORT_FER_PX2 | BITM_PORT_FER_PX3;

   //Reset PHY transceiver (hard reset)
   sc598Eth2ResetPhy(interface);
#endif
}
...
void sc598Eth2InitDmaDesc(NetInterface *interface)
{
   ...
   for(i = 0; i < SC598_ETH2_TX_BUFFER_COUNT; i++)
   {
   ...
   for(i = 0; i < SC598_ETH2_RX_BUFFER_COUNT; i++)
   {
   ...
   *pREG_EMAC1_DMA0_TXDSC_ADDR = adi_rtl_internal_to_system_addr(
      (uint32_t)(uintptr_t) &txDmaDesc[0], 1);

   //Length of the transmit descriptor ring
   *pREG_EMAC1_DMA0_TXDSC_RLEN = SC598_ETH2_TX_BUFFER_COUNT - 1;

   //Start location of the RX descriptor list
   *pREG_EMAC1_DMA0_RXDSC_ADDR = adi_rtl_internal_to_system_addr(
      (uint32_t)(uintptr_t) &rxDmaDesc[0], 1);

   //Length of the receive descriptor ring
   *pREG_EMAC1_DMA0_RXCTL2 = SC598_ETH2_RX_BUFFER_COUNT - 1;
}
...
void sc598Eth2EnableIrq(NetInterface *interface)
{
   ...
   adi_int_EnableInt(INTR_EMAC1_STAT, true);
...
void sc598Eth2DisableIrq(NetInterface *interface)
{
   ...
   adi_int_EnableInt(INTR_EMAC1_STAT, false);
...
void sc598Eth2IrqHandler(uint32_t id, void *param)
{
   ...
   status = *pREG_EMAC1_DMA0_STAT;

   //Packet transmitted?
   if((status & BITM_EMAC_DMA_STAT_TI) != 0)
   {
      //Clear TI interrupt flag
      *pREG_EMAC1_DMA0_STAT = BITM_EMAC_DMA_STAT_TI;
   ...
   if((status & BITM_EMAC_DMA_STAT_RI) != 0)
   {
      ...
      *pREG_EMAC1_DMA0_STAT = BITM_EMAC_DMA_STAT_RI;
   ...
   *pREG_EMAC1_DMA0_STAT = BITM_EMAC_DMA_STAT_NIS;
...
void sc598Eth2EventHandler(NetInterface *interface)
{
   ...
   do
   {
      ...
      error = sc598Eth2ReceivePacket(interface);
...
error_t sc598Eth2SendPacket(NetInterface *interface,
   const NetBuffer *buffer, size_t offset, NetTxAncillary *ancillary)
{
   ...
   if(length > SC598_ETH2_TX_BUFFER_SIZE)
   {
   ...
   *pREG_EMAC1_DMA0_STAT = BITM_EMAC_DMA_STAT_TBU;
   //Instruct the DMA to poll the transmit descriptor list
   *pREG_EMAC1_DMA0_TXDSC_TLPTR = 0;

   //Increment index and wrap around if necessary
   if(++txIndex >= SC598_ETH2_TX_BUFFER_COUNT)
   {
      txIndex = 0;
   }
...
error_t sc598Eth2ReceivePacket(NetInterface *interface)
{
   ...
   if((rxDmaDesc[rxIndex].rdes3 & EMAC_RDES3_OWN) == 0)
   {
         ...
         (rxDmaDesc[rxIndex].rdes3 & EMAC_RDES3_LD) != 0)
      {
         ...
         if((rxDmaDesc[rxIndex].rdes3 & EMAC_RDES3_ES) == 0)
         {
            ...
            n = MIN(n, SC598_ETH2_RX_BUFFER_SIZE);
      ...
      //Increment index and wrap around if necessary
      if(++rxIndex >= SC598_ETH2_RX_BUFFER_COUNT)
      {
         rxIndex = 0;
      }
   }
   ...
   *pREG_EMAC1_DMA0_STAT = BITM_EMAC_DMA_STAT_RBU;
   //Instruct the DMA to poll the receive descriptor list
   *pREG_EMAC1_DMA0_RXDSC_TLPTR = 0;
...
error_t sc598Eth2UpdateMacAddrFilter(NetInterface *interface)
{
   ...
   if(interface->promiscuous)
   {
      ...
      *pREG_EMAC1_MACPKT_FILT = BITM_EMAC_MACPKT_FILT_PR;
   }
   else
   {
      //Set the MAC address of the station
      *pREG_EMAC1_ADDR0_LO = interface->macAddr.w[0] | (interface->macAddr.w[1] << 16);
      *pREG_EMAC1_ADDR0_HI = interface->macAddr.w[2];
      ...
      for(i = 0, j = 0; i < MAC_ADDR_FILTER_SIZE; i++)
      {
         ...
         if(entry->refCount > 0)
         {
            ...
            if(macIsMulticastAddr(&entry->addr))
            {
               ...
               crc = sc598Eth2CalcCrc(&entry->addr, sizeof(MacAddr));
      ...
      if(j >= 1)
      {
         ...
         *pREG_EMAC1_ADDR1_LO = unicastMacAddr[0].w[0] | (unicastMacAddr[0].w[1] << 16);
         *pREG_EMAC1_ADDR1_HI = unicastMacAddr[0].w[2] | BITM_EMAC_ADDR_HI_AE;
      }
      else
      {
         //When the AE bit is cleared, the entry is ignored
         *pREG_EMAC1_ADDR1_LO = 0;
         *pREG_EMAC1_ADDR1_HI = 0;
      }
      ...
      if(interface->acceptAllMulticast)
      {
         ...
         *pREG_EMAC1_MACPKT_FILT = BITM_EMAC_MACPKT_FILT_HPF | BITM_EMAC_MACPKT_FILT_PM;
      }
      else
      {
         //Configure the receive filter
         *pREG_EMAC1_MACPKT_FILT = BITM_EMAC_MACPKT_FILT_HPF | BITM_EMAC_MACPKT_FILT_HMC;

         //Configure the multicast hash table
         *pREG_EMAC1_HASHTBL_REG0 = hashTable[0];
         *pREG_EMAC1_HASHTBL_REG1 = hashTable[1];
...
void sc598Eth2WritePhyReg(uint8_t opcode, uint8_t phyAddr,
   uint8_t regAddr, uint16_t data)
{
   ...
   if(opcode == SMI_OPCODE_WRITE)
   {
      ...
      temp = *pREG_EMAC1_MDIO_ADDR & BITM_EMAC_MDIO_ADDR_CR;
      ...
      *pREG_EMAC1_MDIO_DATA = data & BITM_EMAC_MDIO_DATA_GD;

      //Start a write operation
      *pREG_EMAC1_MDIO_ADDR = temp;
      //Wait for the write to complete
      while((*pREG_EMAC1_MDIO_ADDR & BITM_EMAC_MDIO_ADDR_GB) != 0)
      {
      }
   }
   else
...
uint16_t sc598Eth2ReadPhyReg(uint8_t opcode, uint8_t phyAddr,
   uint8_t regAddr)
{
   uint16_t data;
   uint32_t temp;
   ...
   if(opcode == SMI_OPCODE_READ)
   {
      ...
      *pREG_EMAC1_MDIO_ADDR = temp;
      ...
      //Get register value
      data = *pREG_EMAC1_MDIO_DATA & BITM_EMAC_MDIO_DATA_GD;
   }
   else
   {
      //The MAC peripheral only supports standard Clause 22 opcodes
