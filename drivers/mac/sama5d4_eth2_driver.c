/**
 * @file sama5d4_eth2_driver.c
 * @brief SAMA5D4 Ethernet MAC driver (GMAC1 instance)
 *
 * @section License
 *
...
#include "barriers.h"
#include "core/net.h"
#include "drivers/mac/sama5d4_eth2_driver.h"
#include "debug.h"
...
#pragma location = SAMA5D4_ETH2_RAM_SECTION
static uint8_t txBuffer[SAMA5D4_ETH2_TX_BUFFER_COUNT][SAMA5D4_ETH2_TX_BUFFER_SIZE];
//RX buffer
#pragma data_alignment = 8
#pragma location = SAMA5D4_ETH2_RAM_SECTION
static uint8_t rxBuffer[SAMA5D4_ETH2_RX_BUFFER_COUNT][SAMA5D4_ETH2_RX_BUFFER_SIZE];
//TX buffer descriptors
#pragma data_alignment = 8
#pragma location = SAMA5D4_ETH2_RAM_SECTION
static Sama5d4Eth2TxBufferDesc txBufferDesc[SAMA5D4_ETH2_TX_BUFFER_COUNT];
//RX buffer descriptors
#pragma data_alignment = 8
#pragma location = SAMA5D4_ETH2_RAM_SECTION
static Sama5d4Eth2RxBufferDesc rxBufferDesc[SAMA5D4_ETH2_RX_BUFFER_COUNT];
...
//TX buffer
static uint8_t txBuffer[SAMA5D4_ETH2_TX_BUFFER_COUNT][SAMA5D4_ETH2_TX_BUFFER_SIZE]
   __attribute__((aligned(8), __section__(SAMA5D4_ETH2_RAM_SECTION)));
//RX buffer
static uint8_t rxBuffer[SAMA5D4_ETH2_RX_BUFFER_COUNT][SAMA5D4_ETH2_RX_BUFFER_SIZE]
   __attribute__((aligned(8), __section__(SAMA5D4_ETH2_RAM_SECTION)));
//TX buffer descriptors
...
const NicDriver sama5d4Eth2Driver =
{
   ...
   sama5d4Eth2Init,
   sama5d4Eth2Tick,
   sama5d4Eth2EnableIrq,
   sama5d4Eth2DisableIrq,
   sama5d4Eth2EventHandler,
   sama5d4Eth2SendPacket,
   sama5d4Eth2UpdateMacAddrFilter,
   sama5d4Eth2UpdateMacConfig,
   sama5d4Eth2WritePhyReg,
   sama5d4Eth2ReadPhyReg,
   TRUE,
   TRUE,
   TRUE,
   FALSE
...
/**
 * @brief SAMA5D4 Ethernet MAC initialization
...
error_t sama5d4Eth2Init(NetInterface *interface)
{
   ...
   //Debug message
   TRACE_INFO("Initializing SAMA5D4 Ethernet MAC (GMAC1)...\r\n");

   //Save underlying network interface
   nicDriverInterface = interface;

   //Enable GMAC peripheral clock
   PMC->PMC_PCER1 = (1 << (ID_GMAC1 - 32));

   //Disable transmit and receive circuits
   GMAC1->GMAC_NCR = 0;

   //GPIO configuration
   sama5d4Eth2InitGpio(interface);

   //Configure MDC clock speed
   GMAC1->GMAC_NCFGR = GMAC_NCFGR_CLK_MCK_96;
   //Enable management port (MDC and MDIO)
   GMAC1->GMAC_NCR |= GMAC_NCR_MPE;
   ...
   GMAC1->GMAC_SA[0].GMAC_SAB = interface->macAddr.w[0] | (interface->macAddr.w[1] << 16);
   GMAC1->GMAC_SA[0].GMAC_SAT = interface->macAddr.w[2];

   //The MAC supports 3 additional addresses for unicast perfect filtering
   GMAC1->GMAC_SA[1].GMAC_SAB = 0;
   GMAC1->GMAC_SA[2].GMAC_SAB = 0;
   GMAC1->GMAC_SA[3].GMAC_SAB = 0;
   ...
   GMAC1->GMAC_NCFGR |= GMAC_NCFGR_MAXFS | GMAC_NCFGR_MTIHEN;

   //Initialize buffer descriptors
   sama5d4Eth2InitBufferDesc(interface);

   //Clear transmit status register
   GMAC1->GMAC_TSR = GMAC_TSR_HRESP | GMAC_TSR_UND | GMAC_TSR_TXCOMP |
      GMAC_TSR_TFC | GMAC_TSR_TXGO | GMAC_TSR_RLE | GMAC_TSR_COL |
      GMAC_TSR_UBR;

   //Clear receive status register
   GMAC1->GMAC_RSR = GMAC_RSR_HNO | GMAC_RSR_RXOVR | GMAC_RSR_REC |
      GMAC_RSR_BNA;

   //First disable all GMAC interrupts
   GMAC1->GMAC_IDR = 0xFFFFFFFF;

   //Only the desired ones are enabled
   GMAC1->GMAC_IER = GMAC_IER_HRESP | GMAC_IER_ROVR | GMAC_IER_TCOMP |
      GMAC_IER_TFC | GMAC_IER_RLEX | GMAC_IER_TUR | GMAC_IER_RXUBR |
      GMAC_IER_RCOMP;

   //Read GMAC_ISR register to clear any pending interrupt
   status = GMAC1->GMAC_ISR;
   (void) status;

   //Configure interrupt controller
   AIC->AIC_SSR = ID_GMAC1;
   AIC->AIC_SMR = AIC_SMR_SRCTYPE_INT_LEVEL_SENSITIVE | AIC_SMR_PRIOR(SAMA5D4_ETH2_IRQ_PRIORITY);
   AIC->AIC_SVR = (uint32_t) sama5d4Eth2IrqHandler;

   //Enable the GMAC to transmit and receive data
   GMAC1->GMAC_NCR |= GMAC_NCR_TXEN | GMAC_NCR_RXEN;
...
void sama5d4Eth2InitBufferDesc(NetInterface *interface)
{
   ...
   for(i = 0; i < SAMA5D4_ETH2_TX_BUFFER_COUNT; i++)
   {
   ...
   for(i = 0; i < SAMA5D4_ETH2_RX_BUFFER_COUNT; i++)
   {
   ...
   GMAC1->GMAC_TBQB = (uint32_t) txBufferDesc;
   //Start location of the RX descriptor list
   GMAC1->GMAC_RBQB = (uint32_t) rxBufferDesc;
}


/**
 * @brief SAMA5D4 Ethernet MAC timer handler
 *
...
/**
 * @brief SAMA5D4 Ethernet MAC interrupt service routine
 **/
...
/**
 * @brief SAMA5D4 Ethernet MAC event handler
 * @param[in] interface Underlying network interface
 **/
...
void sama5d4Eth2EventHandler(NetInterface *interface)
{
   ...
   if((rsr & (GMAC_RSR_HNO | GMAC_RSR_RXOVR | GMAC_RSR_REC | GMAC_RSR_BNA)) != 0)
   {
      ...
      do
      {
         ...
         error = sama5d4Eth2ReceivePacket(interface);
...
error_t sama5d4Eth2SendPacket(NetInterface *interface,
   const NetBuffer *buffer, size_t offset, NetTxAncillary *ancillary)
{
   ...
   if(length > SAMA5D4_ETH2_TX_BUFFER_SIZE)
   {
   ...
   if(txBufferIndex < (SAMA5D4_ETH2_TX_BUFFER_COUNT - 1))
   {
   ...
   GMAC1->GMAC_NCR |= GMAC_NCR_TSTART;
...
error_t sama5d4Eth2ReceivePacket(NetInterface *interface)
{
   ...
   for(i = 0; i < SAMA5D4_ETH2_RX_BUFFER_COUNT; i++)
   {
      ...
      if(j >= SAMA5D4_ETH2_RX_BUFFER_COUNT)
      {
         j -= SAMA5D4_ETH2_RX_BUFFER_COUNT;
      }
   ...
   for(i = 0; i < j; i++)
   {
      ...
      if(eofIndex != UINT_MAX && i >= sofIndex && i <= eofIndex)
      {
         ...
         n = MIN(size, SAMA5D4_ETH2_RX_BUFFER_SIZE);
      ...
      //Wrap around to the beginning of the buffer if necessary
      if(rxBufferIndex >= SAMA5D4_ETH2_RX_BUFFER_COUNT)
      {
         rxBufferIndex = 0;
      }
   }
...
error_t sama5d4Eth2UpdateMacAddrFilter(NetInterface *interface)
{
   ...
   if(j >= 1)
   {
      ...
      GMAC1->GMAC_SA[1].GMAC_SAB = unicastMacAddr[0].w[0] | (unicastMacAddr[0].w[1] << 16);
      GMAC1->GMAC_SA[1].GMAC_SAT = unicastMacAddr[0].w[2];
   }
   else
   {
   ...
   {
      //The address is activated when SAT register is written
      GMAC1->GMAC_SA[2].GMAC_SAB = unicastMacAddr[1].w[0] | (unicastMacAddr[1].w[1] << 16);
      GMAC1->GMAC_SA[2].GMAC_SAT = unicastMacAddr[1].w[2];
   }
   else
   {
      //The address is deactivated when SAB register is written
   ...
   {
      //The address is activated when SAT register is written
      GMAC1->GMAC_SA[3].GMAC_SAB = unicastMacAddr[2].w[0] | (unicastMacAddr[2].w[1] << 16);
      GMAC1->GMAC_SA[3].GMAC_SAT = unicastMacAddr[2].w[2];
   }
   else
   {
      //The address is deactivated when SAB register is written
   ...
   if(j >= 4)
   {
      GMAC1->GMAC_NCFGR |= GMAC_NCFGR_UNIHEN;
   }
   else
   {
      GMAC1->GMAC_NCFGR &= ~GMAC_NCFGR_UNIHEN;
   ...
   GMAC1->GMAC_HRB = hashTable[0];
   GMAC1->GMAC_HRT = hashTable[1];
...
void sama5d4Eth2WritePhyReg(uint8_t opcode, uint8_t phyAddr,
   uint8_t regAddr, uint16_t data)
{
   ...
   if(opcode == SMI_OPCODE_WRITE)
   {
      ...
      while((GMAC1->GMAC_NSR & GMAC_NSR_IDLE) == 0)
      {
      }
   }
   else
...
uint16_t sama5d4Eth2ReadPhyReg(uint8_t opcode, uint8_t phyAddr,
   uint8_t regAddr)
{
   uint16_t data;
   uint32_t temp;
   ...
   if(opcode == SMI_OPCODE_READ)
   {
      ...
      //Get register value
      data = GMAC1->GMAC_MAN & GMAC_MAN_DATA_Msk;
   }
   else
   {
      //The MAC peripheral only supports standard Clause 22 opcodes
