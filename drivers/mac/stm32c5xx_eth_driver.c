/**
 * @file stm32c5xx_eth_driver.c
 * @brief STM32C5 Ethernet MAC driver
 *
 * @section License
 *
...
#include "stm32_hal.h"
#include "core/net.h"
#include "drivers/mac/stm32c5xx_eth_driver.h"
#include "debug.h"
...
/**
 * @brief STM32C5 Ethernet MAC initialization
...
error_t stm32c5xxEthInit(NetInterface *interface)
{
   ...
   //Debug message
   TRACE_INFO("Initializing STM32C5 Ethernet MAC...\r\n");

   //Save underlying network interface
   nicDriverInterface = interface;

   //GPIO configuration
   stm32c5xxEthInitGpio(interface);
   ...
   ETH1->MACCR = ETH_MACCR_GPSLCE | ETH_MACCR_RESERVED15 | ETH_MACCR_DO;

   //Set the maximum packet size that can be accepted
   temp = ETH1->MACECR & ~ETH_MACECR_GPSL;
   ETH1->MACECR = temp | STM32C5XX_ETH_RX_BUFFER_SIZE;

   //Configure MAC address filtering
   stm32c5xxEthUpdateMacAddrFilter(interface);
   ...
   ETH1->DMACCR = ETH_DMACCR_DSL_Val(0);

   //Configure TX features
   ETH1->DMACTXCR = ETH_DMACTXCR_TXPBL_Val(32);

   //Configure RX features
   ETH1->DMACRXCR = ETH_DMACRXCR_RXPBL_Val(32) |
      ETH_DMACRXCR_RBSZ_Val(STM32C5XX_ETH_RX_BUFFER_SIZE);

   //Enable store and forward mode
   ETH1->MTLTXQOMR |= ETH_MTLTXQOMR_TSF;
   ETH1->MTLRXQOMR |= ETH_MTLRXQOMR_RSF;

   //Initialize DMA descriptor lists
   stm32c5xxEthInitDmaDesc(interface);

   //Prevent interrupts from being generated when the transmit statistic
   //counters reach half their maximum value
   ETH1->MMC_TX_INTERRUPT_MASK = ETH_MMC_TX_INTERRUPT_MASK_TXLPITRCIM |
      ETH_MMC_TX_INTERRUPT_MASK_TXLPIUSCIM | ETH_MMC_TX_INTERRUPT_MASK_TXGPKTIM |
      ETH_MMC_TX_INTERRUPT_MASK_TXMCOLGPIM | ETH_MMC_TX_INTERRUPT_MASK_TXSCOLGPIM;

   //Prevent interrupts from being generated when the receive statistic
   //counters reach half their maximum value
   ETH1->MMC_RX_INTERRUPT_MASK = ETH_MMC_RX_INTERRUPT_MASK_RXLPITRCIM |
      ETH_MMC_RX_INTERRUPT_MASK_RXLPIUSCIM | ETH_MMC_RX_INTERRUPT_MASK_RXUCGPIM |
      ETH_MMC_RX_INTERRUPT_MASK_RXALGNERPIM | ETH_MMC_RX_INTERRUPT_MASK_RXCRCERPIM;

   //Disable MAC interrupts
   ETH1->MACIER = 0;
   //Enable the desired DMA interrupts
   ETH1->DMACIER = ETH_DMACIER_NIE | ETH_DMACIER_RIE | ETH_DMACIER_TIE;

   //Set priority grouping (4 bits for pre-emption priority, no bits for subpriority)
   NVIC_SetPriorityGrouping(STM32C5XX_ETH_IRQ_PRIORITY_GROUPING);

   //Configure Ethernet interrupt priority
   NVIC_SetPriority(ETH1_IRQn, NVIC_EncodePriority(STM32C5XX_ETH_IRQ_PRIORITY_GROUPING,
      STM32C5XX_ETH_IRQ_GROUP_PRIORITY, STM32C5XX_ETH_IRQ_SUB_PRIORITY));

   //Enable MAC transmission and reception
   ETH1->MACCR |= ETH_MACCR_TE | ETH_MACCR_RE;

   //Enable DMA transmission and reception
   ETH1->DMACTXCR |= ETH_DMACTXCR_ST;
   ETH1->DMACRXCR |= ETH_DMACRXCR_SR;
...
{
//Nucleo-C5A3ZG evaluation board?
#if defined(USE_NUCLEO_C5A3ZG)
   ...
   //Configure ETH1_RMII_TXD1 (PG12)
   gpio_config.alternate = HAL_GPIO_AF_10;
   HAL_GPIO_Init(HAL_GPIOG, HAL_GPIO_PIN_12, &gpio_config);
...
void stm32c5xxEthInitDmaDesc(NetInterface *interface)
{
   ...
   for(i = 0; i < STM32C5XX_ETH_TX_BUFFER_COUNT; i++)
   {
   ...
   for(i = 0; i < STM32C5XX_ETH_RX_BUFFER_COUNT; i++)
   {
   ...
   ETH1->DMACTXDLAR = (uint32_t) &txDmaDesc[0];
   //Length of the transmit descriptor ring
   ETH1->DMACTXRLR = STM32C5XX_ETH_TX_BUFFER_COUNT - 1;

   //Start location of the RX descriptor list
   ETH1->DMACRXDLAR = (uint32_t) &rxDmaDesc[0];
   //Length of the receive descriptor ring
   ETH1->DMACRXRLR = STM32C5XX_ETH_RX_BUFFER_COUNT - 1;
}


/**
 * @brief STM32C5 Ethernet MAC timer handler
 *
...
/**
 * @brief STM32C5 Ethernet MAC event handler
 * @param[in] interface Underlying network interface
 **/
...
void stm32c5xxEthEventHandler(NetInterface *interface)
{
   ...
   do
   {
      ...
      error = stm32c5xxEthReceivePacket(interface);
   ...
   const NetBuffer *buffer, size_t offset, NetTxAncillary *ancillary)
{
   ...
   if(length > STM32C5XX_ETH_TX_BUFFER_SIZE)
   {
   ...
   ETH1->DMACSR = ETH_DMACSR_TBU;
   //Instruct the DMA to poll the transmit descriptor list
   ETH1->DMACTXDTPR = 0;

   //Increment index and wrap around if necessary
   if(++txIndex >= STM32C5XX_ETH_TX_BUFFER_COUNT)
   {
      txIndex = 0;
   }
...
error_t stm32c5xxEthReceivePacket(NetInterface *interface)
{
   ...
   if((rxDmaDesc[rxIndex].rdes3 & ETH_RDES3_OWN) == 0)
   {
         ...
         (rxDmaDesc[rxIndex].rdes3 & ETH_RDES3_LD) != 0)
      {
         ...
         if((SBS->PMCR & SBS_PMCR_ETH1_SEL_PHY) != HAL_SBS_ETHPHY_ITF_GMII_MII)
         {
            status &= ~ETH_RDES3_DE;
         }
         ...
         if(status == 0)
         {
            ...
            n = MIN(n, STM32C5XX_ETH_RX_BUFFER_SIZE);
      ...
      //Increment index and wrap around if necessary
      if(++rxIndex >= STM32C5XX_ETH_RX_BUFFER_COUNT)
      {
         rxIndex = 0;
      }
   }
      ...
      for(i = 0, j = 0; i < MAC_ADDR_FILTER_SIZE; i++)
      {
         ...
         if(entry->refCount > 0)
         {
            ...
            if(macIsMulticastAddr(&entry->addr))
            {
               ...
               crc = stm32c5xxEthCalcCrc(&entry->addr, sizeof(MacAddr));
...
void stm32c5xxEthWritePhyReg(uint8_t opcode, uint8_t phyAddr,
   uint8_t regAddr, uint16_t data)
{
   ...
   if(opcode == SMI_OPCODE_WRITE)
   {
      ...
      temp |= ETH_MACMDIOAR_GOC_Val(1) | ETH_MACMDIOAR_MB;
      //PHY address
      ...
      ETH1->MACMDIODR = data & ETH_MACMDIODR_MD;

      //Start a write operation
      ETH1->MACMDIOAR = temp;
      //Wait for the write to complete
      while((ETH1->MACMDIOAR & ETH_MACMDIOAR_MB) != 0)
      {
      }
   }
   else
...
uint16_t stm32c5xxEthReadPhyReg(uint8_t opcode, uint8_t phyAddr,
   uint8_t regAddr)
{
   uint16_t data;
   uint32_t temp;
   ...
   if(opcode == SMI_OPCODE_READ)
   {
      ...
      temp |= ETH_MACMDIOAR_GOC_Val(3) | ETH_MACMDIOAR_MB;
      //PHY address
      temp |= (phyAddr << 21) & ETH_MACMDIOAR_PA;
      //Register address
      temp |= (regAddr << 16) & ETH_MACMDIOAR_RDA;
      ...
      //Get register value
      data = ETH1->MACMDIODR & ETH_MACMDIODR_MD;
   }
   else
   {
      //The MAC peripheral only supports standard Clause 22 opcodes
