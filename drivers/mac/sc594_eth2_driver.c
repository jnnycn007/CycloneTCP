...
#include "core/net.h"
#include "drivers/mac/sc594_eth2_driver.h"
#include "debug.h"
...
#pragma location = SC594_ETH2_RAM_SECTION
static uint8_t txBuffer[SC594_ETH2_TX_BUFFER_COUNT][SC594_ETH2_TX_BUFFER_SIZE];
//Receive buffer
#pragma data_alignment = 4
#pragma location = SC594_ETH2_RAM_SECTION
static uint8_t rxBuffer[SC594_ETH2_RX_BUFFER_COUNT][SC594_ETH2_RX_BUFFER_SIZE];
//Transmit DMA descriptors
#pragma data_alignment = 8
#pragma location = SC594_ETH2_RAM_SECTION
static Sc594Eth2TxDmaDesc txDmaDesc[SC594_ETH2_TX_BUFFER_COUNT];
//Receive DMA descriptors
#pragma data_alignment = 8
#pragma location = SC594_ETH2_RAM_SECTION
static Sc594Eth2RxDmaDesc rxDmaDesc[SC594_ETH2_RX_BUFFER_COUNT];
...
//Transmit buffer
static uint8_t txBuffer[SC594_ETH2_TX_BUFFER_COUNT][SC594_ETH2_TX_BUFFER_SIZE]
   __attribute__((aligned(4), __section__(SC594_ETH2_RAM_SECTION)));
//Receive buffer
static uint8_t rxBuffer[SC594_ETH2_RX_BUFFER_COUNT][SC594_ETH2_RX_BUFFER_SIZE]
   __attribute__((aligned(4), __section__(SC594_ETH2_RAM_SECTION)));
//Transmit DMA descriptors
static Sc594Eth2TxDmaDesc txDmaDesc[SC594_ETH2_TX_BUFFER_COUNT]
   __attribute__((aligned(8), __section__(SC594_ETH2_RAM_SECTION)));
//Receive DMA descriptors
static Sc594Eth2RxDmaDesc rxDmaDesc[SC594_ETH2_RX_BUFFER_COUNT]
   __attribute__((aligned(8), __section__(SC594_ETH2_RAM_SECTION)));
...
static Sc594Eth2TxDmaDesc *txCurDmaDesc;
//Pointer to the current RX DMA descriptor
static Sc594Eth2RxDmaDesc *rxCurDmaDesc;
...
const NicDriver sc594Eth2Driver =
{
   ...
   sc594Eth2EnableIrq,
   sc594Eth2DisableIrq,
   sc594Eth2EventHandler,
   sc594Eth2SendPacket,
   sc594Eth2UpdateMacAddrFilter,
   sc594Eth2UpdateMacConfig,
   sc594Eth2WritePhyReg,
   sc594Eth2ReadPhyReg,
   TRUE,
   TRUE,
   TRUE,
...
/**
 * @brief ADSP-SC594 Ethernet MAC initialization
...
error_t sc594Eth2Init(NetInterface *interface)
{
   ...
   //Debug message
   TRACE_INFO("Initializing ADSP-SC594 Ethernet MAC (EMAC1)...\r\n");

   //Save underlying network interface
   nicDriverInterface = interface;

   //GPIO configuration
   sc594Eth2InitGpio(interface);
   ...
   *pREG_EMAC1_MACCFG = BITM_EMAC_MACCFG_PS | BITM_EMAC_MACCFG_DO;
   ...
   sc594Eth2UpdateMacAddrFilter(interface);
   ...
   *pREG_EMAC1_DMA0_BUSMODE = BITM_EMAC_DMA0_BUSMODE_AAL |
   ...
   sc594Eth2InitDmaDesc(interface);
   ...
   *pREG_EMAC1_DMA0_IEN = BITM_EMAC_DMA0_IEN_NIE | BITM_EMAC_DMA0_IEN_RIE |
   ...
   adi_int_InstallHandler(INTR_EMAC1_DMA0, sc594Eth2IrqHandler, interface,
      false);
...
{
//EV-SC594-SOM evaluation board?
#if defined(USE_EV_SC594_SOM)
   uint32_t temp;
...
void sc594Eth2InitDmaDesc(NetInterface *interface)
{
   ...
   for(i = 0; i < SC594_ETH2_TX_BUFFER_COUNT; i++)
   {
   ...
   for(i = 0; i < SC594_ETH2_RX_BUFFER_COUNT; i++)
   {
      //The descriptor is initially owned by the DMA
      rxDmaDesc[i].rdes0 = EMAC_RDES0_OWN;
      //Use chain structure rather than ring structure
      rxDmaDesc[i].rdes1 = EMAC_RDES1_RCH | (SC594_ETH2_RX_BUFFER_SIZE & EMAC_RDES1_RBS1);
...
/**
 * @brief ADSP-SC594 Ethernet MAC timer handler
 *
...
void sc594Eth2EnableIrq(NetInterface *interface)
{
   ...
   adi_int_EnableInt(INTR_EMAC1_DMA0, true);
...
void sc594Eth2DisableIrq(NetInterface *interface)
{
   ...
   adi_int_EnableInt(INTR_EMAC1_DMA0, false);
...
/**
 * @brief ADSP-SC594 Ethernet MAC interrupt service routine
...
/**
 * @brief ADSP-SC594 Ethernet MAC event handler
 * @param[in] interface Underlying network interface
 **/
...
void sc594Eth2EventHandler(NetInterface *interface)
{
   ...
   do
   {
      ...
      error = sc594Eth2ReceivePacket(interface);
   ...
   const NetBuffer *buffer, size_t offset, NetTxAncillary *ancillary)
{
   ...
   if(length > SC594_ETH2_TX_BUFFER_SIZE)
   {
   ...
   *pREG_EMAC1_DMA0_TXPOLL = 0;
   ...
   txCurDmaDesc = (Sc594Eth2TxDmaDesc *) adi_rtl_system_to_internal_addr(
      txCurDmaDesc->tdes3);
   ...
   if((rxCurDmaDesc->rdes0 & EMAC_RDES0_OWN) == 0)
   {
         ...
         (rxCurDmaDesc->rdes0 & EMAC_RDES0_LS) != 0)
      {
         ...
         if((rxCurDmaDesc->rdes0 & EMAC_RDES0_ES) == 0)
         {
            ...
            n = MIN(n, SC594_ETH2_RX_BUFFER_SIZE);
      ...
      //Point to the next descriptor in the list
      rxCurDmaDesc = (Sc594Eth2RxDmaDesc *) adi_rtl_system_to_internal_addr(
         rxCurDmaDesc->rdes3);
   }
   else
   {
      ...
      for(i = 0, j = 0; i < MAC_ADDR_FILTER_SIZE; i++)
      {
         ...
         if(entry->refCount > 0)
         {
            ...
            if(macIsMulticastAddr(&entry->addr))
            {
               ...
               crc = sc594Eth2CalcCrc(&entry->addr, sizeof(MacAddr));
