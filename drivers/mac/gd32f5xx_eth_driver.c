/**
 * @file gd32f5xx_eth_driver.c
 * @brief GigaDevice GD32F5 Ethernet MAC driver
 *
 * @section License
 *
...
#include "gd32f5xx.h"
#include "core/net.h"
#include "drivers/mac/gd32f5xx_eth_driver.h"
#include "debug.h"
...
static uint8_t txBuffer[GD32F5XX_ETH_TX_BUFFER_COUNT][GD32F5XX_ETH_TX_BUFFER_SIZE];
//Receive buffer
#pragma data_alignment = 4
static uint8_t rxBuffer[GD32F5XX_ETH_RX_BUFFER_COUNT][GD32F5XX_ETH_RX_BUFFER_SIZE];
//Transmit DMA descriptors
#pragma data_alignment = 4
static Gd32f5xxTxDmaDesc txDmaDesc[GD32F5XX_ETH_TX_BUFFER_COUNT];
//Receive DMA descriptors
#pragma data_alignment = 4
static Gd32f5xxRxDmaDesc rxDmaDesc[GD32F5XX_ETH_RX_BUFFER_COUNT];
...
static Gd32f5xxTxDmaDesc *txCurDmaDesc;
//Pointer to the current RX DMA descriptor
static Gd32f5xxRxDmaDesc *rxCurDmaDesc;


/**
 * @brief GD32F5 Ethernet MAC driver
 **/

const NicDriver gd32f5xxEthDriver =
{
   NIC_TYPE_ETHERNET,
   ETH_MTU,
   gd32f5xxEthInit,
   gd32f5xxEthTick,
   gd32f5xxEthEnableIrq,
   gd32f5xxEthDisableIrq,
   gd32f5xxEthEventHandler,
   gd32f5xxEthSendPacket,
   gd32f5xxEthUpdateMacAddrFilter,
   gd32f5xxEthUpdateMacConfig,
   gd32f5xxEthWritePhyReg,
   gd32f5xxEthReadPhyReg,
   TRUE,
   TRUE,
   TRUE,
   FALSE
...
/**
 * @brief GD32F5 Ethernet MAC initialization
...
error_t gd32f5xxEthInit(NetInterface *interface)
{
   ...
   //Debug message
   TRACE_INFO("Initializing GD32F5 Ethernet MAC...\r\n");

   //Save underlying network interface
   nicDriverInterface = interface;

   //GPIO configuration
   gd32f5xxEthInitGpio(interface);
   ...
   gd32f5xxEthInitDmaDesc(interface);
   ...
   NVIC_SetPriorityGrouping(GD32F5XX_ETH_IRQ_PRIORITY_GROUPING);

   //Configure Ethernet interrupt priority
   NVIC_SetPriority(ENET_IRQn, NVIC_EncodePriority(GD32F5XX_ETH_IRQ_PRIORITY_GROUPING,
      GD32F5XX_ETH_IRQ_GROUP_PRIORITY, GD32F5XX_ETH_IRQ_SUB_PRIORITY));
...
__weak_func void gd32f5xxEthInitGpio(NetInterface *interface)
{
//GD32F527I-EVAL evaluation board?
#if defined(USE_GD32F527I_EVAL)
   //Enable SYSCFG clock
...
void gd32f5xxEthInitDmaDesc(NetInterface *interface)
{
   ...
   for(i = 0; i < GD32F5XX_ETH_TX_BUFFER_COUNT; i++)
   {
   ...
   for(i = 0; i < GD32F5XX_ETH_RX_BUFFER_COUNT; i++)
   {
      //The descriptor is initially owned by the DMA
      rxDmaDesc[i].rdes0 = ENET_RDES0_DAV;
      //Use chain structure rather than ring structure
      rxDmaDesc[i].rdes1 = ENET_RDES1_RCHM | (GD32F5XX_ETH_RX_BUFFER_SIZE & ENET_RDES1_RB1S);
...
/**
 * @brief GD32F5 Ethernet MAC timer handler
 *
...
/**
 * @brief GD32F5 Ethernet MAC interrupt service routine
 **/
...
/**
 * @brief GD32F5 Ethernet MAC event handler
 * @param[in] interface Underlying network interface
 **/
...
void gd32f5xxEthEventHandler(NetInterface *interface)
{
   ...
   do
   {
      ...
      error = gd32f5xxEthReceivePacket(interface);
...
error_t gd32f5xxEthSendPacket(NetInterface *interface,
   const NetBuffer *buffer, size_t offset, NetTxAncillary *ancillary)
{
   ...
   if(length > GD32F5XX_ETH_TX_BUFFER_SIZE)
   {
   ...
   txCurDmaDesc = (Gd32f5xxTxDmaDesc *) txCurDmaDesc->tdes3;
   ...
   if((rxCurDmaDesc->rdes0 & ENET_RDES0_DAV) == 0)
   {
         ...
         (rxCurDmaDesc->rdes0 & ENET_RDES0_LDES) != 0)
      {
         ...
         if((rxCurDmaDesc->rdes0 & ENET_RDES0_ERRS) == 0)
         {
            ...
            n = MIN(n, GD32F5XX_ETH_RX_BUFFER_SIZE);
      ...
      //Point to the next descriptor in the list
      rxCurDmaDesc = (Gd32f5xxRxDmaDesc *) rxCurDmaDesc->rdes3;
   }
   else
   {
      //No more data in the receive buffer
   ...
   for(i = 0, j = 0; i < MAC_ADDR_FILTER_SIZE; i++)
   {
      ...
      if(entry->refCount > 0)
      {
         ...
         if(macIsMulticastAddr(&entry->addr))
         {
            ...
            crc = gd32f5xxEthCalcCrc(&entry->addr, sizeof(MacAddr));
...
void gd32f5xxEthWritePhyReg(uint8_t opcode, uint8_t phyAddr,
...
uint16_t gd32f5xxEthReadPhyReg(uint8_t opcode, uint8_t phyAddr,
   uint8_t regAddr)
{
   uint16_t data;
   uint32_t temp;
