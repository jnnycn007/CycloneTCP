...
#ifndef _SAMA5D4_ETH2_DRIVER_H
#define _SAMA5D4_ETH2_DRIVER_H

//Dependencies
#include "core/nic.h"

//Number of TX buffers
#ifndef SAMA5D4_ETH2_TX_BUFFER_COUNT
   #define SAMA5D4_ETH2_TX_BUFFER_COUNT 8
#elif (SAMA5D4_ETH2_TX_BUFFER_COUNT < 1)
   #error SAMA5D4_ETH2_TX_BUFFER_COUNT parameter is not valid
#endif

//TX buffer size
#ifndef SAMA5D4_ETH2_TX_BUFFER_SIZE
   #define SAMA5D4_ETH2_TX_BUFFER_SIZE 1536
#elif (SAMA5D4_ETH2_TX_BUFFER_SIZE != 1536)
   #error SAMA5D4_ETH2_TX_BUFFER_SIZE parameter is not valid
#endif

//Number of RX buffers
#ifndef SAMA5D4_ETH2_RX_BUFFER_COUNT
   #define SAMA5D4_ETH2_RX_BUFFER_COUNT 96
#elif (SAMA5D4_ETH2_RX_BUFFER_COUNT < 12)
   #error SAMA5D4_ETH2_RX_BUFFER_COUNT parameter is not valid
#endif

//RX buffer size
#ifndef SAMA5D4_ETH2_RX_BUFFER_SIZE
   #define SAMA5D4_ETH2_RX_BUFFER_SIZE 128
#elif (SAMA5D4_ETH2_RX_BUFFER_SIZE != 128)
   #error SAMA5D4_ETH2_RX_BUFFER_SIZE parameter is not valid
#endif

//Ethernet interrupt priority
#ifndef SAMA5D4_ETH2_IRQ_PRIORITY
   #define SAMA5D4_ETH2_IRQ_PRIORITY 0
#elif (SAMA5D4_ETH2_IRQ_PRIORITY < 0)
   #error SAMA5D4_ETH2_IRQ_PRIORITY parameter is not valid
#endif

//Name of the section where to place DMA buffers
#ifndef SAMA5D4_ETH2_RAM_SECTION
   #define SAMA5D4_ETH2_RAM_SECTION ".region_nocache"
#endif
...
typedef struct
{
   uint32_t address;
   uint32_t status;
} Sama5d4Eth2RxBufferDesc;


//SAMA5D4 Ethernet MAC driver (GMAC1 instance)
extern const NicDriver sama5d4Eth2Driver;

//SAMA5D4 Ethernet MAC related functions
error_t sama5d4Eth2Init(NetInterface *interface);
void sama5d4Eth2InitGpio(NetInterface *interface);
