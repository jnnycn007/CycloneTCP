...
#include "NuMicro.h"
#include "core/net.h"
#include "drivers/mac/m5531_eth_driver.h"
#include "debug.h"
...
#pragma location = M5531_ETH_RAM_SECTION
static uint8_t txBuffer[M5531_ETH_TX_BUFFER_COUNT][M5531_ETH_TX_BUFFER_SIZE];
//Receive buffer
#pragma data_alignment = 4
#pragma location = M5531_ETH_RAM_SECTION
static uint8_t rxBuffer[M5531_ETH_RX_BUFFER_COUNT][M5531_ETH_RX_BUFFER_SIZE];
//Transmit DMA descriptors
#pragma data_alignment = 8
#pragma location = M5531_ETH_RAM_SECTION
static M5531TxDmaDesc txDmaDesc[M5531_ETH_TX_BUFFER_COUNT];
//Receive DMA descriptors
#pragma data_alignment = 8
#pragma location = M5531_ETH_RAM_SECTION
static M5531RxDmaDesc rxDmaDesc[M5531_ETH_RX_BUFFER_COUNT];
...
//Transmit buffer
static uint8_t txBuffer[M5531_ETH_TX_BUFFER_COUNT][M5531_ETH_TX_BUFFER_SIZE]
   __attribute__((aligned(4), __section__(M5531_ETH_RAM_SECTION)));
//Receive buffer
static uint8_t rxBuffer[M5531_ETH_RX_BUFFER_COUNT][M5531_ETH_RX_BUFFER_SIZE]
   __attribute__((aligned(4), __section__(M5531_ETH_RAM_SECTION)));
//Transmit DMA descriptors
static M5531TxDmaDesc txDmaDesc[M5531_ETH_TX_BUFFER_COUNT]
   __attribute__((aligned(8), __section__(M5531_ETH_RAM_SECTION)));
//Receive DMA descriptors
static M5531RxDmaDesc rxDmaDesc[M5531_ETH_RX_BUFFER_COUNT]
   __attribute__((aligned(8), __section__(M5531_ETH_RAM_SECTION)));
...
static M5531TxDmaDesc *txCurDmaDesc;
//Pointer to the current RX DMA descriptor
static M5531RxDmaDesc *rxCurDmaDesc;


/**
 * @brief M5531 Ethernet MAC driver
 **/
...
const NicDriver m5531EthDriver =
{
   ...
   m5531EthEnableIrq,
   m5531EthDisableIrq,
   m5531EthEventHandler,
   m5531EthSendPacket,
   m5531EthUpdateMacAddrFilter,
   m5531EthUpdateMacConfig,
   m5531EthWritePhyReg,
   m5531EthReadPhyReg,
   TRUE,
   TRUE,
...
/**
 * @brief M5531 Ethernet MAC initialization
...
error_t m5531EthInit(NetInterface *interface)
{
   ...
   //Debug message
   TRACE_INFO("Initializing M5531 Ethernet MAC...\r\n");

   //Save underlying network interface
   nicDriverInterface = interface;

   //GPIO configuration
   m5531EthInitGpio(interface);
   ...
   m5531EthUpdateMacAddrFilter(interface);
   ...
   NVIC_SetPriorityGrouping(M5531_ETH_IRQ_PRIORITY_GROUPING);

   //Configure Ethernet interrupt priority
   NVIC_SetPriority(EMAC0_IRQn, NVIC_EncodePriority(M5531_ETH_IRQ_PRIORITY_GROUPING,
      M5531_ETH_IRQ_GROUP_PRIORITY, M5531_ETH_IRQ_SUB_PRIORITY));
...
{
//NuMaker-X-M55M1D evaluation board?
#if defined(USE_NUMAKER_X_M55M1D)
   //Enable GPIO clocks
   ...
   GPIO_SetSlewCtl(PE, BIT10 | BIT11 | BIT12, GPIO_SLEWCTL_FAST1);
#endif
}
...
void m5531EthInitDmaDesc(NetInterface *interface)
{
   ...
   for(i = 0; i < M5531_ETH_TX_BUFFER_COUNT; i++)
   {
   ...
   for(i = 0; i < M5531_ETH_RX_BUFFER_COUNT; i++)
   {
      //The descriptor is initially owned by the DMA
      rxDmaDesc[i].rdes0 = EMAC_RDES0_OWN;
      //Use chain structure rather than ring structure
      rxDmaDesc[i].rdes1 = EMAC_RDES1_RCH | (M5531_ETH_RX_BUFFER_SIZE & EMAC_RDES1_RBS1);
...
/**
 * @brief M5531 Ethernet MAC timer handler
 *
...
/**
 * @brief M5531 Ethernet MAC interrupt service routine
 **/
...
/**
 * @brief M5531 Ethernet MAC event handler
 * @param[in] interface Underlying network interface
 **/
...
void m5531EthEventHandler(NetInterface *interface)
{
   ...
   do
   {
      ...
      error = m5531EthReceivePacket(interface);
   ...
   const NetBuffer *buffer, size_t offset, NetTxAncillary *ancillary)
{
   ...
   if(length > M5531_ETH_TX_BUFFER_SIZE)
   {
   ...
   txCurDmaDesc = (M5531TxDmaDesc *) txCurDmaDesc->tdes3;
   ...
   if((rxCurDmaDesc->rdes0 & EMAC_RDES0_OWN) == 0)
   {
         ...
         (rxCurDmaDesc->rdes0 & EMAC_RDES0_LS) != 0)
      {
         ...
         if((rxCurDmaDesc->rdes0 & EMAC_RDES0_ES) == 0)
         {
            ...
            n = MIN(n, M5531_ETH_RX_BUFFER_SIZE);
      ...
      //Point to the next descriptor in the list
      rxCurDmaDesc = (M5531RxDmaDesc *) rxCurDmaDesc->rdes3;
   }
   else
   {
      //No more data in the receive buffer
