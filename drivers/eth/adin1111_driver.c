...
/**
 * @brief ADIN1111 controller initialization
...
error_t adin1111Init(NetInterface *interface)
{
   ...
   //Debug message
   TRACE_INFO("Initializing ADIN1111 Ethernet controller...\r\n");
   ...
   adin1111WriteReg(interface, ADIN1111_RESET, ADIN1111_RESET_SWRESET);
   ...
   do
   {
      ...
      //register
      value = adin1111ReadReg(interface, ADIN1111_PHYID);

      //If the reset value of the register can be read, the device has exited
      //reset and is ready for configuration
   } while(value != (ADIN1111_PHYID_OUI_DEFAULT | ADIN1111_PHYID_MODEL_DEFAULT |
      ADIN1111_PHYID_REVISION_DEFAULT));
   ...
   do
   {
      ...
      value = adin1111ReadReg(interface, ADIN1111_STATUS0);

      //Check the value of the RESETC bit
   } while((value & ADIN1111_STATUS0_RESETC) == 0);

   //Write 1 to the RESETC field in the STATUS0 register to clear this field
   adin1111WriteReg(interface, ADIN1111_STATUS0, ADIN1111_STATUS0_RESETC);
   ...
   do
   {
      ...
      value = adin1111ReadMmdReg(interface, ADIN1111_CRSM_STAT);

      //Check the value of the CRSM_SYS_RDY bit
   } while((value & ADIN1111_CRSM_STAT_CRSM_SYS_RDY) == 0);

   //Dump SPI registers for debugging purpose
   adin1111DumpReg(interface);
   //Dump PHY registers for debugging purpose
   adin1111DumpPhyReg(interface);

   //Configure MAC address filtering
   adin1111UpdateMacAddrFilter(interface);

#if (ADIN1111_OA_SPI_SUPPORT == ENABLED)
   //Configure the SPI protocol engine
   adin1111WriteReg(interface, ADIN1111_CONFIG0, ADIN1111_CONFIG0_CSARFE |
      ADIN1111_CONFIG0_ZARFE | ADIN1111_CONFIG0_TXCTHRESH_16_CREDITS |
      ADIN1111_CONFIG0_CPS_64B);
#else
   //Enable store and forward mode
   value = adin1111ReadReg(interface, ADIN1111_CONFIG0);
   value &= ~(ADIN1111_CONFIG0_TXCTE | ADIN1111_CONFIG0_RXCTE);
   adin1111WriteReg(interface, ADIN1111_CONFIG0, value);
#endif

   //Enable CRC append in the MAC TX path
   value = adin1111ReadReg(interface, ADIN1111_CONFIG2);
   value |= ADIN1111_CONFIG2_CRC_APPEND;
   adin1111WriteReg(interface, ADIN1111_CONFIG2, value);

   //Clear IMASK0 register
   adin1111WriteReg(interface, ADIN1111_IMASK0, 0xFFFFFFFF);

#if (ADIN1111_OA_SPI_SUPPORT == ENABLED)
   //Disable generic SPI protocol interrupts
   adin1111WriteReg(interface, ADIN1111_IMASK1, 0xFFFFFFFF);
#else
   //Write the IMASK1 register to enable interrupts as required
   adin1111WriteReg(interface, ADIN1111_IMASK1, ~(ADIN1111_IMASK1_TX_RDY_MASK |
      ADIN1111_IMASK1_P1_RX_RDY_MASK | ADIN1111_IMASK1_LINK_CHANGE_MASK));
#endif
   ...
   value = adin1111ReadReg(interface, ADIN1111_CONFIG0);
   value |= ADIN1111_CONFIG0_SYNC;
   adin1111WriteReg(interface, ADIN1111_CONFIG0, value);

   //Enable LED1 output
   value = adin1111ReadMmdReg(interface, ADIN1111_DIGIO_PINMUX);
   value &= ~ADIN1111_DIGIO_PINMUX_DIGIO_LED1_PINMUX;
   value |= ADIN1111_DIGIO_PINMUX_DIGIO_LED1_PINMUX_LED_1;
   adin1111WriteMmdReg(interface, ADIN1111_DIGIO_PINMUX, value);

   //Configure LED0 and LED1 function
   adin1111WriteMmdReg(interface, ADIN1111_LED_CNTRL,
      ADIN1111_LED_CNTRL_LED0_EN |
      ADIN1111_LED_CNTRL_LED0_FUNCTION_LINKUP_TXRX_ACTIVITY |
      ADIN1111_LED_CNTRL_LED1_EN |
      ADIN1111_LED_CNTRL_LED1_FUNCTION_MASTER);

   //Set LED0 and LED1 polarity
   adin1111WriteMmdReg(interface, ADIN1111_LED_POLARITY,
      ADIN1111_LED_POLARITY_LED0_POLARITY_AUTOSENSE |
      ADIN1111_LED_POLARITY_LED1_POLARITY_AUTOSENSE);

   //Perform custom configuration
   adin1111InitHook(interface);
   ...
   value = adin1111ReadMmdReg(interface, ADIN1111_CRSM_SFT_PD_CNTRL);
   value &= ~ADIN1111_CRSM_SFT_PD_CNTRL_CRSM_SFT_PD;
   adin1111WriteMmdReg(interface, ADIN1111_CRSM_SFT_PD_CNTRL, value);
...
/**
 * @brief ADIN1111 custom configuration
 * @param[in] interface Underlying network interface
 **/
...
void adin1111Tick(NetInterface *interface)
{
   ...
   value = adin1111ReadReg(interface, ADIN1111_STATUS1);
   //Retrieve current link state
   linkState = (value & ADIN1111_STATUS1_P1_LINK_STATUS) ? TRUE : FALSE;
...
/**
 * @brief ADIN1111 interrupt service routine
 * @param[in] interface Underlying network interface
...
bool_t adin1111IrqHandler(NetInterface *interface)
{
   ...
   mask = adin1111ReadReg(interface, ADIN1111_IMASK1);
   //Disable interrupts to release the interrupt line
   adin1111WriteReg(interface, ADIN1111_IMASK1, 0xFFFFFFFF);

   //Read interrupt status register
   status = adin1111ReadReg(interface, ADIN1111_STATUS1);

   //Link status changed interrupt?
   if((status & ADIN1111_STATUS1_LINK_CHANGE) != 0)
   {
      //Disable link status changed interrupt
      mask |= ADIN1111_IMASK1_LINK_CHANGE_MASK;
   ...
   if((status & ADIN1111_STATUS1_TX_RDY) != 0)
   {
   ...
   //Packet received?
   if((status & ADIN1111_STATUS1_P1_RX_RDY) != 0)
   {
      //Disable P1_RX_RDY interrupt
      mask |= ADIN1111_IMASK1_P1_RX_RDY_MASK;
   ...
   adin1111WriteReg(interface, ADIN1111_IMASK1, mask);
...
void adin1111EventHandler(NetInterface *interface)
{
   ...
   status = adin1111ReadReg(interface, ADIN1111_BUFSTS);

   //Process all the data chunks currently available
   while((status & ADIN1111_BUFSTS_RCA) != 0)
   {
      //Read incoming packet
      adin1111ReceivePacket(interface);
   ...
   {
      //Clear interrupt flag
      adin1111WriteReg(interface, ADIN1111_STATUS1,
         ADIN1111_STATUS1_LINK_CHANGE);

      //Check link state
      if((status & ADIN1111_STATUS1_P1_LINK_STATUS) != 0)
      {
         //The PHY is only able to operate in 10 Mbps mode
         interface->linkSpeed = NIC_LINK_SPEED_10MBPS;
         interface->duplexMode = NIC_FULL_DUPLEX_MODE;
   ...
   if((status & ADIN1111_STATUS1_P1_RX_RDY) != 0)
   {
      ...
      } while((status & ADIN1111_STATUS1_P1_RX_RDY) != 0);
   }
   ...
   const NetBuffer *buffer, size_t offset, NetTxAncillary *ancillary)
{
   ...
   n = (status & ADIN1111_BUFSTS_TXC) >> 8;

   //Check the number of transmit credits available
   if(length <= (n * ADIN1111_CHUNK_PAYLOAD_SIZE))
   {
      //A data transaction consists of multiple chunks
      for(i = 0; i < length; i += n)
      {
         //The default size of the data chunk payload is 64 bytes
         n = MIN(length - i, ADIN1111_CHUNK_PAYLOAD_SIZE);

         //Set up a data transfer
         header = ADIN1111_TX_HEADER_DNC | ADIN1111_TX_HEADER_NORX |
            ADIN1111_TX_HEADER_DV;
         ...
         if((i + n) == length)
         {
            ...
            header |= ((n - 1) << 8) & ADIN1111_TX_HEADER_EBO;
         }

         //The parity bit is calculated over the transmit data header
         if(adin1111CalcParity(header) != 0)
         {
            header |= ADIN1111_CTRL_HEADER_P;
         }
         ...
         //by a 4-byte footer
         footer = LOAD32BE(chunk + ADIN1111_CHUNK_PAYLOAD_SIZE);

         //The RCA field indicates the number of receive data chunks available
         if((footer & ADIN1111_RX_FOOTER_RCA) != 0)
         {
   ...
   return NO_ERROR;
#else
   static uint8_t temp[ADIN1111_ETH_TX_BUFFER_SIZE];
   size_t n;
   size_t length;
   ...
   if(length > ADIN1111_ETH_TX_BUFFER_SIZE)
   {
      //The transmitter can accept another packet
      osSetEvent(&interface->nicTxEvent);
      //Report an error
...
error_t adin1111ReceivePacket(NetInterface *interface)
{
   ...
   while(1)
   {
      ...
      if((length + ADIN1111_CHUNK_PAYLOAD_SIZE) > ADIN1111_ETH_RX_BUFFER_SIZE)
      {
      ...
      if((footer & ADIN1111_RX_FOOTER_DV) == 0)
      {
      ...
      if(length == 0)
      {
         if((footer & ADIN1111_RX_FOOTER_SV) == 0)
         {
      ...
      if((footer & ADIN1111_RX_FOOTER_EV) != 0)
      {
         n = ((footer & ADIN1111_RX_FOOTER_EBO) >> 8) + 1;
      }
      else
      {
         n = ADIN1111_CHUNK_PAYLOAD_SIZE;
      }
   ...
   return error;
#else
   static uint8_t temp[ADIN1111_ETH_RX_BUFFER_SIZE];
   error_t error;
   size_t length;
   uint16_t header;

   //Get the size of the frame at the head of the RX FIFO in bytes
   length = adin1111ReadReg(interface, ADIN1111_P1_RX_FSIZE) &
      ADIN1111_P1_RX_FSIZE_P1_RX_FRM_SIZE;

   //Any packet pending in the receive buffer?
   if(length >= ADIN1111_FRAME_HEADER_SIZE)
   {
      NetRxAncillary ancillary;
      ...
      length = MIN(length, ADIN1111_ETH_RX_BUFFER_SIZE);
      //Additional options can be passed to the stack along with the packet
      ancillary = NET_DEFAULT_RX_ANCILLARY;
...
error_t adin1111UpdateMacAddrFilter(NetInterface *interface)
{
   ...
   adin1111WriteReg(interface, ADIN1111_ADDR_FILT_UPRn(0),
      ADIN1111_ADDR_FILT_UPR_APPLY2PORT1 | ADIN1111_ADDR_FILT_UPR_TO_HOST |
      ADIN1111_ADDR_FILT_UPR_MAC_ADDR_47_32);

   //Set the lower 32 bits of the broadcast MAC address
   adin1111WriteReg(interface, ADIN1111_ADDR_FILT_LWRn(0),
      ADIN1111_ADDR_FILT_LWR_MAC_ADDR_31_0);

   //Set the upper 16 bits of the station MAC address
   adin1111WriteReg(interface, ADIN1111_ADDR_FILT_UPRn(1),
      ADIN1111_ADDR_FILT_UPR_APPLY2PORT1 | ADIN1111_ADDR_FILT_UPR_TO_HOST |
      (interface->macAddr.b[0] << 8) | interface->macAddr.b[1]);

   //Set the lower 32 bits of the station MAC address
   adin1111WriteReg(interface, ADIN1111_ADDR_FILT_LWRn(1),
      ...
      j < ADIN1111_ADDR_TABLE_SIZE; i++)
   {
      ...
      if(entry->refCount > 0)
      {
         ...
         adin1111WriteReg(interface, ADIN1111_ADDR_FILT_UPRn(j),
            ADIN1111_ADDR_FILT_UPR_APPLY2PORT1 | ADIN1111_ADDR_FILT_UPR_TO_HOST |
            (entry->addr.b[0] << 8) | entry->addr.b[1]);

         //Set the lower 32 bits of the current MAC address
         adin1111WriteReg(interface, ADIN1111_ADDR_FILT_LWRn(j),
   ...
   for(; j < ADIN1111_ADDR_TABLE_SIZE; j++)
   {
      //Clear current MAC address
      adin1111WriteReg(interface, ADIN1111_ADDR_FILT_UPRn(j), 0);
      adin1111WriteReg(interface, ADIN1111_ADDR_FILT_LWRn(j), 0);
   }
   ...
   uint32_t data)
{
   ...
   header = ADIN1111_CTRL_HEADER_WNR | ADIN1111_CTRL_HEADER_AID;

   //The MMS field selects the specific register memory map to access
   if(address < 0x30)
   {
      header |= (ADIN1111_MMS_STD << 24) & ADIN1111_CTRL_HEADER_MMS;
   }
   else
   {
      header |= (ADIN1111_MMS_MAC << 24) & ADIN1111_CTRL_HEADER_MMS;
   }

   //Address of the first register to access
   header |= (address << 8) & ADIN1111_CTRL_HEADER_ADDR;
   //Specifies the number of registers to write
   header |= (0 << 1) & ADIN1111_CTRL_HEADER_LEN;
...
#if (ADIN1111_PROTECTION_SUPPORT == ENABLED)
   ...
   //Write command
   interface->spiDriver->transfer(ADIN1111_SPI_CMD_WRITE | (address >> 8));
...
uint32_t adin1111ReadReg(NetInterface *interface, uint16_t address)
{
   ...
   header = ADIN1111_CTRL_HEADER_AID;
   ...
   //Write command
   interface->spiDriver->transfer(ADIN1111_SPI_CMD_READ | (address >> 8));
   interface->spiDriver->transfer(address & 0xFF);
   ...
   uint16_t data)
{
   ...
   value = ADIN1111_MDIOACC_MDIO_ST_CLAUSE_22 | ADIN1111_MDIOACC_MDIO_OP_WRITE;
   //Set PHY address
   value |= ADIN1111_MDIOACC_MDIO_PRTAD_DEFAULT;
   //Set register address
   value |= (address << 16) & ADIN1111_MDIOACC_MDIO_DEVAD;
   //Set register value
   value |= data & ADIN1111_MDIOACC_MDIO_DATA;

   //Write MDIOACC0 register
   adin1111WriteReg(interface, ADIN1111_MDIOACC0, value);

   //Poll MDIOACC0.TRDONE to determine that the write operation has completed
   do
   {
      //Read MDIOACC0 register
      value = adin1111ReadReg(interface, ADIN1111_MDIOACC0);

      //When the MDIO transaction completes, the TRDONE bit is set to 1
   } while((value & ADIN1111_MDIOACC_MDIO_TRDONE) == 0);
}
...
uint16_t adin1111ReadPhyReg(NetInterface *interface, uint8_t address)
{
   ...
   value = ADIN1111_MDIOACC_MDIO_ST_CLAUSE_22 | ADIN1111_MDIOACC_MDIO_OP_READ;
   //Set PHY address
   value |= ADIN1111_MDIOACC_MDIO_PRTAD_DEFAULT;
   //Set register address
   value |= (address << 16) & ADIN1111_MDIOACC_MDIO_DEVAD;
   ...
   return value & ADIN1111_MDIOACC_MDIO_DATA;
}
...
void adin1111DumpPhyReg(NetInterface *interface)
{
   ...
   for(i = 0; i < 32; i++)
   {
      ...
      TRACE_DEBUG("%02" PRIu8 ": 0x%04" PRIX16 "\r\n", i,
         adin1111ReadPhyReg(interface, i));
   }
   ...
   uint16_t regAddr, uint16_t data)
{
   ...
   value = ADIN1111_MDIOACC_MDIO_ST_CLAUSE_45 | ADIN1111_MDIOACC_MDIO_OP_ADDR;
   //MDIO_PRTAD is always written to 1
   value |= ADIN1111_MDIOACC_MDIO_PRTAD_DEFAULT;
   //Set device address
   value |= (devAddr << 16) & ADIN1111_MDIOACC_MDIO_DEVAD;
   //Set register address
   value |= regAddr & ADIN1111_MDIOACC_MDIO_DATA;

   //Write MDIOACC0 register
   adin1111WriteReg(interface, ADIN1111_MDIOACC0, value);

   //Perform a Clause 45 write operation
   value = ADIN1111_MDIOACC_MDIO_ST_CLAUSE_45 | ADIN1111_MDIOACC_MDIO_OP_WRITE;
   //MDIO_PRTAD is always written to 1
   value |= ADIN1111_MDIOACC_MDIO_PRTAD_DEFAULT;
   //Set device address
   value |= (devAddr << 16) & ADIN1111_MDIOACC_MDIO_DEVAD;
   ...
   adin1111WriteReg(interface, ADIN1111_MDIOACC1, value);

   //Poll MDIOACC1.TRDONE to determine that the write operation has completed
   do
   {
      //Read MDIOACC1 register
      value = adin1111ReadReg(interface, ADIN1111_MDIOACC1);
   ...
   uint16_t regAddr)
{
   ...
   value = ADIN1111_MDIOACC_MDIO_ST_CLAUSE_45 | ADIN1111_MDIOACC_MDIO_OP_READ;
   //MDIO_PRTAD is always written to 1
   value |= ADIN1111_MDIOACC_MDIO_PRTAD_DEFAULT;
   //Set device address
   value |= (devAddr << 16) & ADIN1111_MDIOACC_MDIO_DEVAD;
...
{
#if (ADIN1111_OA_SPI_SUPPORT == DISABLED)
   size_t i;
   ...
   //Write command
   interface->spiDriver->transfer(ADIN1111_SPI_CMD_WRITE | (ADIN1111_TX >> 8));
   interface->spiDriver->transfer(ADIN1111_TX & 0xFF);
   ...
   for(; ((i + ADIN1111_FRAME_HEADER_SIZE) % 4) != 0; i++)
   {
      interface->spiDriver->transfer(0x00);
   }
   ...
   uint8_t *data, size_t length)
{
   ...
   //Write command
   interface->spiDriver->transfer(ADIN1111_SPI_CMD_READ | (ADIN1111_P1_RX >> 8));
   interface->spiDriver->transfer(ADIN1111_P1_RX & 0xFF);
   ...
   *header |= interface->spiDriver->transfer(0x00);
   ...
   //Read frame data
   for(i = 0; i < length && i < ADIN1111_ETH_RX_BUFFER_SIZE; i++)
   {
